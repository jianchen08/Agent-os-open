"""
端口配置一致性测试 — 验证 0.2 架构下各配置文件端口正确且一致
...
class TestFrontendEnvExample:
    ...
    def test_api_base_url_port(self, content):
        """VITE_API_BASE_URL 应使用端口 9100（0.2 内核）"""
        assert "VITE_API_BASE_URL=http://127.0.0.1:9100" in content, (
            ".env.example 中 VITE_API_BASE_URL 应使用内核端口 9100"
        )
