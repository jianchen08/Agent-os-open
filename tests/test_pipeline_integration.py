...
class TestRestoreChain:
    ...
    async def test_restore_creates_engines_for_running_tasks(self) -> None:
        ...
        5. send_pipeline_message(PipelineMessage(type=MessageType.CHAT, content="continue", pipeline_id="restore-1")) → 成功
        """
        ...
        # 5. 发送消息验证成功
        result = await send_pipeline_message(PipelineMessage(type=MessageType.CHAT, content="continue", pipeline_id="restore-1"))
        assert result.success is True
