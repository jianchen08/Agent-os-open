...
class TestRestoreChain:
    """验证应用重启后管道恢复链条（持有者 TaskWorker.restore_running_pipelines）。

    覆盖场景：restore_running_pipelines 恢复 running 任务、
    未注册且无历史的管道发送失败、跳过已注册管道。
    """
    ...
    async def test_restore_creates_engines_for_running_tasks(self) -> None:
        """验证 restore_running_pipelines 恢复 running 任务。

        测试步骤：
        1. mock task_service.list_tasks 返回 running 任务
        2. mock ServiceProvider 返回路由表等
        3. 调用 TaskExecutorMixin.restore_running_pipelines（通过轻量载体）
        4. 断言 registry.get("restore-1") 存在
        """
        from pipeline.registry import get_engine_registry
        from infrastructure.task_executor import TaskExecutorMixin

        # 轻量载体：给 mixin 一个 _task_service 属性即可调 restore_running_pipelines
        class _Carrier(TaskExecutorMixin):
            pass

        carrier = _Carrier()
        # list_tasks 按 status 区分：running 返回任务，pending 返回空
        def _list_tasks(status):
            if status == "running":
                return [{"pipeline_id": "restore-1", "task_id": "task-1"}]
            return []
        carrier._task_service = MagicMock(list_tasks=MagicMock(side_effect=_list_tasks))
        ...
        mock_provider.get_all_services.return_value = {}

        # register_pipeline 已是合并形态，mock 它避免真实构造引擎
        with patch("infrastructure.service_provider.get_service_provider", return_value=mock_provider):
            with patch.object(get_engine_registry(), "register_pipeline") as mock_register:
                mock_register.return_value = MagicMock()  # 非 None 表示注册成功

                restored = await carrier.restore_running_pipelines()

        assert restored >= 1
        # 验证调用了 register_pipeline
        mock_register.assert_called_once()

    @pytest.mark.asyncio
    async def test_unregistered_pipeline_with_no_history_fails(self) -> None:
        """验证未注册且无历史的管道发送消息失败（I4：send 直接拒绝）。

        测试步骤：
        1. 不注册任何引擎
        2. send_pipeline_message 发送消息到不存在的管道
        3. 断言 result.success == False、method == "rejected"
        """
        from pipeline.message_bus import send_pipeline_message

        # I4 改造：未注册直接拒绝，不再走 revive（_try_revive_pipeline 已删除）
        result = await send_pipeline_message(PipelineMessage(type=MessageType.CHAT, content="hello", pipeline_id="nonexistent-pipe"))
    ...
    async def test_restore_skips_already_registered_pipelines(self) -> None:
        """验证 restore_running_pipelines 跳过已在 registry 中的管道。

        测试步骤：
        1. 预注册一个引擎
        2. mock task_service 返回同 pipeline_id 的任务
        3. 调用 restore_running_pipelines，断言不会重复创建
        """
        from pipeline.registry import get_engine_registry
        from infrastructure.task_executor import TaskExecutorMixin

        # 预注册引擎（registry.get 会命中，restore 应跳过）
        _register_mock_engine("already-exists", is_running=True)

        class _Carrier(TaskExecutorMixin):
            pass

        carrier = _Carrier()
        carrier._task_service = MagicMock(
