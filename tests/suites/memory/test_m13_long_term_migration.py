...
# ═══════════════════════════════════════════════════════════
# dependency_validator 去 ORM
# ═══════════════════════════════════════════════════════════


class TestDependencyValidatorNoORM:
    """M13-a2: dependency_validator 不依赖 ORM。"""

    @pytest.mark.asyncio
    async def test_valid_deps(self) -> None:
        """合法依赖验证通过。"""
        svc = _make_svc()
        t1 = svc.create_task(title="t1")
        from tasks.dependency_validator import DependencyValidator
        v = DependencyValidator(svc)
        result = await v.validate("new", [t1.id])
        assert result.is_valid

    @pytest.mark.asyncio
    async def test_self_dep(self) -> None:
        """自我依赖检测。"""
        svc = _make_svc()
        from tasks.dependency_validator import DependencyValidator
        v = DependencyValidator(svc)
        result = await v.validate("me", ["me"])
        assert not result.is_valid
        assert any("不能依赖自己" in e for e in result.errors)
    ...
    async def test_missing_dep(self) -> None:
        ...
        from tasks.dependency_validator import DependencyValidator
        v = DependencyValidator(svc)
        result = await v.validate("new", ["ghost"])
        assert not result.is_valid

    @pytest.mark.asyncio
    async def test_parent_as_dep(self) -> None:
        """依赖父任务检测。"""
        svc = _make_svc()
        parent = svc.create_task(title="p")
        from tasks.dependency_validator import DependencyValidator
        v = DependencyValidator(svc)
        result = await v.validate("child", [parent.id], parent_task_id=parent.id)
        assert not result.is_valid

    def test_no_sqlalchemy_import(self) -> None:
        """源码中无 sqlalchemy / AsyncSession 导入。"""
        import inspect
        import tasks.dependency_validator as mod
        src = inspect.getsource(mod)
        assert "sqlalchemy" not in src
        assert "AsyncSession" not in src

    @pytest.mark.asyncio
    async def test_duplicate_dep_warning(self) -> None:
        """重复依赖产生 warning（不报错）。"""
        svc = _make_svc()
        t1 = svc.create_task(title="t1")
        from tasks.dependency_validator import DependencyValidator
        v = DependencyValidator(svc)
        result = await v.validate("new", [t1.id, t1.id])
        assert result.is_valid
