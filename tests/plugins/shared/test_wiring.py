...
验证内容：
1. bind_capability_caller 剥掉能力前缀（tool-executor.invoke → invoke），非前缀方法原样透传；
...
# tests/plugins/shared/test_wiring.py → parents[3] = 仓库根
_SHARED_ROOT = Path(__file__).resolve().parents[3] / "plugins" / "shared"
_HINDSIGHT_MEMORY_DIR = _SHARED_ROOT / "system" / "hindsight_memory"
if str(_HINDSIGHT_MEMORY_DIR) not in sys.path:
    sys.path.insert(0, str(_HINDSIGHT_MEMORY_DIR))


def _load_module() -> Any:
    """动态加载 wiring.py（每次新建，避免模块级 sys.path 污染累积）。"""
    mod_name = "hindsight_wiring_test"
    path = _SHARED_ROOT / "wiring.py"
...
def test_module_self_registers_hindsight_dir_on_sys_path(monkeypatch: pytest.MonkeyPatch) -> None:
    """hindsight_memory 目录不在 sys.path 时，wiring 导入自插（memory_backend 可达性）。"""
    hindsight_dir = str(_HINDSIGHT_MEMORY_DIR)
    monkeypatch.setattr(sys, "path", [p for p in sys.path if p != hindsight_dir])

    assert hindsight_dir not in sys.path
    _load_module()
    assert hindsight_dir in sys.path
