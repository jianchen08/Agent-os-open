...
"""host_context HTTP 面缺口补测（业务逻辑归 plugin.py 自身测试）：
...
- http.handle 补充分支：subscribe 非法 body 容错、preview 索引解析与
  代理失败 502、未知路由 404、_fetch_preview 非 200/非 PNG/成功/空端点四态
"""
...
_PLUGIN_DIR = Path(__file__).resolve().parents[4] / "plugins" / "shared" / "pipeline" / "input" / "host_context"


def _load_server() -> Any:
    spec = importlib.util.spec_from_file_location(
        "host_context_gaps_test_server",
        str(_PLUGIN_DIR / "server.py"),
    )
    assert spec is not None and spec.loader is not None
    mod = importlib.util.module_from_spec(spec)
    sys.modules["host_context_gaps_test_server"] = mod
...
class FakeInst:
    """HostContextPlugin 替身：记录调用并回放脚本化返回。"""

    execute_result: object = {"state_updates": {}}
    preview_endpoint = "http://fake-host:9600"
...
async def test_preview_routes(server: Any, fake: FakeInst, monkeypatch: pytest.MonkeyPatch) -> None:
    base = "/ext/pipeline_host_context/preview"

    # 代理失败 → 502（_fetch_preview 收到实例配置的宿主端点 + index）
    async def _none(endpoint: str, index: int) -> None:
    ...
    assert seen["endpoint"] == "http://fake-host:9600", "预览代理目标来自实例配置"
    ...
    async def _png(endpoint: str, index: int) -> bytes | None:
        seen["index"] = index
        return b"\x89PNG\r\n\x1a\n" + b"rest"
...
async def test_unknown_route_404(server: Any, fake: FakeInst) -> None:
    miss = await server.http_handle(path="/ext/pipeline_host_context/nope", method="GET")
...
async def test_fetch_preview_real_function_three_states(
    ...
    """_fetch_preview 本体：空端点 → None；非 200 → None；非 PNG → None；成功 → PNG 字节。"""
    class FakeResp:
    ...
    png = b"\x89PNG\r\n\x1a\n" + b"img"
    endpoint = "http://host-9600:9600"
    urls: list[str] = []

    # 端点为空（配置关闭预览）→ 不发请求直接 None
    assert await server._fetch_preview("", 0) is None
    assert urls == []

    install(FakeResp(200, png))
    assert await server._fetch_preview(endpoint, 0) == png
    assert urls == [f"{endpoint}/selection/preview"], "代理目标 = 配置端点 + /selection/preview"

    install(FakeResp(500, png))
    assert await server._fetch_preview(endpoint, 0) is None
...
class TestPluginIdentityAndBroadcast:
    def test_name_and_priority(self) -> None:
        """插件身份常量：name=host_context、priority=50（引用注入链位置契约）。"""
        from plugin import HostContextPlugin

        p = HostContextPlugin()

        assert p.name == "host_context"
    ...
    async def test_reference_content_renders_position_only_when_present(
        ...
        assert result.state_updates["host.injected_for"] == "msg-1"
