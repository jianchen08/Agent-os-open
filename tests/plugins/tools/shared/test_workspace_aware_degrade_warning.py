...
_TOOLS_DIR = Path(__file__).resolve().parents[4] / "plugins" / "shared" / "tools"  # plugins/shared/tools/
...
def test_checker_failure_denies_and_warns_once(monkeypatch, caplog):
    ...
    monkeypatch.setattr(mod.WorkspaceAwareMixin, "_policy_manager", None)
    monkeypatch.setattr(mod, "_policy_unavailable_warned", False)
    # 让已真实导入的 SDK permission_checker 抛错：接缝是模块属性，
    # 不 mock 内部方法、不改实现文件。
    import agentos_plugin_sdk.permission_checker as _pc_mod
