...
"""godot_context HTTP 面 wire 形状锁（http_json 收敛替换的契约锚）。
...
``{"error": <str>}`` + 对应 HTTP 状态。业务语义由 test_godot_context_plugin.py
...
_PLUGIN_DIR = Path(__file__).resolve().parents[1] / "plugins" / "shared" / "pipeline" / "input" / "godot_context"


@pytest.fixture
def server() -> Any:
    spec = importlib.util.spec_from_file_location(
        "godot_context_http_shape_test_server",
        str(_PLUGIN_DIR / "server.py"),
    )
    assert spec is not None
    assert spec.loader is not None
    mod = importlib.util.module_from_spec(spec)
    sys.modules["godot_context_http_shape_test_server"] = mod
...
def test_error_shape_400_invalid_json_body(server: Any, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(server, "get_instance", lambda: object.__new__(server.GodotContextPlugin))
    status, body = _decode(
        _call(server, "/ext/pipeline_godot_context/selection", "POST", raw_body="not-json")
    )
    assert status == 400
...
def test_error_shape_403_wrong_shared_secret(server: Any, monkeypatch: pytest.MonkeyPatch) -> None:
    """设置共享密钥后，错误 X-Godot-Secret → 403 {"error": "forbidden"}。"""
    monkeypatch.setattr(server, "get_instance", lambda: object.__new__(server.GodotContextPlugin))
    monkeypatch.setenv("GODOT_CONTEXT_SHARED_SECRET", "s3cret")
    status, body = _decode(
        _call(server, "/ext/pipeline_godot_context/selection", "POST", raw_body="{}",
              headers={"X-Godot-Secret": "wrong"})
    )
    assert status == 403
...
def test_error_shape_404_unknown_route(server: Any, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(server, "get_instance", lambda: object.__new__(server.GodotContextPlugin))
    status, body = _decode(_call(server, "/ext/pipeline_godot_context/nope", "GET"))
    assert status == 404
    assert body == {"error": "unknown route GET /ext/pipeline_godot_context/nope"}


def test_success_shape_subscribe(server: Any, monkeypatch: pytest.MonkeyPatch) -> None:
    """200 成功路径：业务 dict 原样入 body（锁定 json 序列化形状）。"""
    monkeypatch.setattr(server, "get_instance", lambda: server.GodotContextPlugin())
    status, body = _decode(
        _call(server, "/ext/pipeline_godot_context/subscribe", "POST", raw_body='{"thread_id": "t1"}')
    )
    assert status == 200
