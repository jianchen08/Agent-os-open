...
class TestMessageBus:
    ...
    async def test_send_pipeline_message_returns_inject_result(self):
        """验证 send_pipeline_message 返回 InjectResult。

        mock _find_engine 返回 (None, "")，模拟引擎不存在场景，
        断言返回 InjectResult 且 success == False。
        """
        with (
            patch("pipeline.message_bus._find_engine", return_value=(None, "")),
            patch(
                "pipeline.message_bus._try_revive_pipeline",
            ...
            result = await send_pipeline_message("nonexistent", "test")
    ...
    async def test_send_pipeline_message_rejects_empty_pipeline_id(self):
        ...
        """
        result = await send_pipeline_message("", "test")
...
class TestRegisterThenSend:
    ...
    async def test_register_then_send_message_flow(self):
        ...
        with patch(
            ...
            result = await send_pipeline_message("flow-123", "hello")
    ...
    async def test_send_to_unregistered_pipeline_fails(self):
        ...
        with (
            ...
            result = await send_pipeline_message("unregistered-pipeline", "hello")
