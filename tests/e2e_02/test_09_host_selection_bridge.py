...
"""
E2E 测试：宿主选中引用桥经真内核的推送链路。

回归背景：内核 dispatcher（http_dispatcher.rs dispatch_http）把请求体
base64 编码后传给插件 http.handle；host_context 服务端曾直接
json.loads(raw_body)，导致宿主插件的一切推送恒 400
"invalid json"，选中引用链路整体断裂（单测直调 handle_push 无法暴露）。

验证：
  1. POST /ext/pipeline_host_context/selection（明文 JSON over HTTP，
...
运行前提：内核 9100 已启动（pipeline_host_context 默认启用）、
Godot 编辑器无需运行（推送由测试直接模拟，连接者=缺省 godot 配置）。
...
_SELECTION_URL = "http://127.0.0.1:9100/ext/pipeline_host_context/selection"
