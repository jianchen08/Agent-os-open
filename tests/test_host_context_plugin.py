...
def test_push_selection_emits_to_subscribed_threads():
    """选中推送（签名变化）→ 对订阅线程 emit host_selection_changed。"""
    emitter = FakeEmitter()
    gc_plugin.set_emitter(emitter)
    p = gc_plugin.HostContextPlugin()
    p.subscribe("t1")
    ...
    assert event == "host_selection_changed"
...
def test_snapshot_heartbeat_timeout_marks_offline():
    ...
    asyncio.run(p.handle_push(_selection_payload()))
    p._last_push_ms -= gc_plugin.HostContextPlugin.HEARTBEAT_STALE_MS + 1
...
def test_execute_merges_reference_into_last_user_message(messages):
    ...
    assert result.state_updates["host.injected_for"] == "m1"
...
def test_execute_skips_when_message_has_other_source_reference():
    """消息已含其他 source 的引用块（前端多 provider 拼接）→ 同样不二次合并。

    回环判定按引用块形态（'<reference source="'）而非具体来源——来源是
    配置可变的，按形态判定才不会随配置漂移失效。
    """
    gc_plugin.set_emitter(None)
    p = gc_plugin.HostContextPlugin()
    asyncio.run(p.handle_push(_selection_payload()))

    other_block = '<reference source="design" layer="cover">\n- 封面 (frame) @ Figma/Cover\n</reference>'
    messages = [{"role": "user", "content": "改一下\n\n" + other_block, "seq": 3}]
    result = _run_execute(p, {"message_id": "m9", "messages": messages})
...
def test_execute_merge_idempotent_both_guards():
    ...
    state["host.injected_for"] = r1.state_updates["host.injected_for"]
...
_PUSH_PATH = "/ext/pipeline_host_context/selection"
_SUB_PATH = "/ext/pipeline_host_context/subscribe"
...
def _load_server_module():
    ...
    spec = importlib.util.spec_from_file_location("host_context_server", server_py)
...
def _fresh_instance():
    ...
    单测绕过内核注入，manifest fields.default 不生效——预览端点需显式给配置
    （source/display_name 走代码内 godot 缺省回退，与运行时注入语义一致）。
    """
    server_mod._instance = server_mod.HostContextPlugin(
        config={"preview_endpoint": "http://127.0.0.1:9600"})
...
def _http(method: str, path: str, raw_body: str = "", query: dict | None = None,
    ...
    kw: dict[str, object] = {
        "path": path,
        "method": method,
        "plugin_id": "pipeline_host_context",
        "raw_body": raw_body,
    }
...
def test_http_subscribe_base64_then_push_broadcasts():
    ...
    assert emitter.calls[-1][0] == "host_selection_changed"
...
def test_dismiss_recovers_on_reselect_same_signature():
    """dismiss 后用户在宿主重新点选同节点（type=selection 同签名）→ 引用恢复并广播。"""
    emitter = FakeEmitter()
    gc_plugin.set_emitter(emitter)
    p = gc_plugin.HostContextPlugin()
    p.subscribe("t1")
...
# ── S4：共享密钥门控（HOST_CONTEXT_SHARED_SECRET 设置后匿名推送 403）──


def test_http_push_with_secret_required_missing_header_forbidden(monkeypatch):
    """设置共享密钥后，无 X-Host-Secret 头的匿名推送必须 403（选中内容会
...
def test_http_push_without_secret_env_unchanged(monkeypatch):
    ...
    monkeypatch.delenv("HOST_CONTEXT_SHARED_SECRET", raising=False)
    server_mod.set_emitter(None)
    _fresh_instance()
...
def test_on_load_builds_singleton_without_frontend_capability():
    ...
    assert isinstance(inst, server_mod.HostContextPlugin)
...
def test_execute_tool_injects_reference_via_plugin_result():
    ...
    assert out["state_updates"]["host.injected_for"] == "m1"
...
def test_fetch_preview_returns_png_only(monkeypatch, status, data, expected):
    ...
    endpoint = _fresh_instance().preview_endpoint

    out = asyncio.run(server_mod._fetch_preview(endpoint, 3))

    assert out == expected
    # 请求形态：代理到实例配置端点 /selection/preview，index 透传
    url, params = fake.last_request
    assert url == f"{endpoint}/selection/preview"
...
def test_fetch_preview_connection_error_returns_none(monkeypatch):
    ...
    endpoint = _fresh_instance().preview_endpoint
    assert asyncio.run(server_mod._fetch_preview(endpoint, 0)) is None


_PREVIEW_PATH = "/ext/pipeline_host_context/preview"
...
def test_http_preview_unavailable_returns_502(monkeypatch):
    ...
    _install_fake_aiohttp(monkeypatch, exc=OSError("host down"))
    server_mod.set_emitter(None)
    _fresh_instance()
...
@pytest.mark.parametrize(
    ("method", "path"),
    [("GET", "/ext/pipeline_host_context/whatever"), ("PATCH", _PUSH_PATH)],
...
# ── 连接者身份配置化：source/display_name/preview_endpoint 流转 ──


@pytest.mark.parametrize(
    "cfg",
    [{"source": "vscode", "display_name": "VSCode", "preview_endpoint": "http://127.0.0.1:9741"},
     {"source": "blender", "display_name": "", "preview_endpoint": ""}],
    ids=["vscode-full", "blender-fallback"],
)
def test_config_identity_flows_into_snapshot_and_reference_block(cfg):
    """配置的连接者身份 → 快照携带 source/display_name；引用块 source 用配置值。

    display_name 留空回退 source；两套区分配置证非硬编码单例。
    """
    p = gc_plugin.HostContextPlugin(config=cfg)
    asyncio.run(p.handle_push(_selection_payload()))
    snap = p.snapshot()
    expect_source = cfg["source"]
    expect_name = cfg["display_name"] or cfg["source"]
    assert snap["source"] == expect_source
    assert snap["display_name"] == expect_name

    merged = p._build_reference_content(snap)
    assert merged.startswith(f'<reference source="{expect_source}" scene="res://demo_main.tscn">')


def test_config_namespaced_settings_form_kernel_contract():
    """内核注入形态：config 按 config_files[].id 命名空间嵌套（{"settings": {...}}）。

    锁与 invoker build_injected_config 的契约——namespace dict 存在时以其为
    真值源，平铺形态（直构/测试）保持兼容。
    """
    p = gc_plugin.HostContextPlugin(config={
        "settings": {"source": "vscode", "display_name": "VSCode",
                     "preview_endpoint": "http://127.0.0.1:9741"},
    })
    asyncio.run(p.handle_push(_selection_payload()))
    snap = p.snapshot()
    assert snap["source"] == "vscode"
    assert snap["display_name"] == "VSCode"
    assert p.preview_endpoint == "http://127.0.0.1:9741"


def test_config_empty_preview_endpoint_disables_proxy(monkeypatch):
    """preview_endpoint 留空 → _fetch_preview 不发请求直接 None（预览关闭）。"""
    import sys
    from types import SimpleNamespace

    def _boom(*_a, **_k):
        raise AssertionError("端点为空不得发起 HTTP 请求")

    monkeypatch.setitem(sys.modules, "aiohttp", SimpleNamespace(
        ClientTimeout=_boom, ClientSession=_boom))

    p = gc_plugin.HostContextPlugin(config={"preview_endpoint": ""})
    assert asyncio.run(server_mod._fetch_preview(p.preview_endpoint, 0)) is None
