...
> 口径冲突时以本文件 + `docs/decisions/` 为准（README / ARCHITECTURE / guides 已对齐 0.2 现状）。
...
（普通 clone 无此行为，但提交纪律通用）。任何修改（含中间产物、测试数据）完成后**立即
...
- **一切皆插件（架构公理，写文档/方案一律以此为基）**：内核只是执行基座（管道解释
  ...
  能力→插件对照表见 `docs/ARCHITECTURE.md` 设计哲学 §1。
...
- **权限模式**：4 档权限模式（default/accept_edits/auto/bypass，黑名单制：未命中
  ...
  该默认由旁路档承载，与显式选旁路同档同路径，OBS-R255-1 2026-09-22 统一）；
  显式选 default/accept_edits/auto 三档后以所选档为准（隔离不再豁免黑名单），
  显式选旁路即免审批本身；选择器如实显示当前生效档（未显式选择时隔离/worktree
  显示「免审批」，与显式选旁路可辨别）。非隔离会话无免审批默认，按档走。
