...
fn test_user_journey_multi_turn_state_continuity() {
    ...
    let entry2 = reg.get_or_init(
        TENANT,
        "pipe_journey",
        "thread_42",
        "agentos",
        make_state(&["should_not_use"]),
    );
    ...
    assert!(
        !reg.contains("tenant_b", "pipe_x"),
        "租户 B 的 pipe_x 不应可见租户 A 的注册"
    );
    ...
    assert_eq!(
        a.read().state["messages"].as_array().unwrap()[0]["content"],
        "a-msg"
    );
    assert_eq!(
        b.read().state["messages"].as_array().unwrap()[0]["content"],
        "b-msg"
    );
}
