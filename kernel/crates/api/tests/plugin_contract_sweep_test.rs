...
fn check_types_supported(v: &Value, path: String, errors: &mut Vec<String>) {
    const SUPPORTED: &[&str] = &["object", "string", "boolean", "integer", "number", "array", "null"];
    if let Value::Object(m) = v {
        for (k, sub) in m {
            if k == "type" {
                if let Some(t) = sub.as_str() {
                    if !SUPPORTED.contains(&t) {
                        errors.push(format!("{path}/type: 不支持的 type '{t}'（执行器关键字子集）"));
                    }
                }
            }
...
fn all_plugin_contracts_are_executor_consumable() {
    ...
    for mf in &manifests {
            ...
            .flatten()
        {
            ...
            if let Some(schema) = ins {
                collect_patterns(schema, format!("tools.{name}.input_schema"), &mut tool_patterns);
                check_types_supported(schema, format!("tools.{name}.input_schema"), &mut type_errors);
                // required 元素必须是字符串
            ...
            if let Some(schema) = outs {
                collect_patterns(schema, format!("tools.{name}.output_schema"), &mut tool_patterns);
                check_types_supported(schema, format!("tools.{name}.output_schema"), &mut type_errors);
            }
            if tool_patterns.iter().any(|(p, _)| p.starts_with("tools.")) {
                ...
                let has_input_pattern = ins
                    .and_then(|s| s.get("properties"))
                    .and_then(|p| p.as_object())
                    .is_some_and(|props| props.values().any(|v| {
                        v.get("pattern").is_some()
                    }));
        ...
        {
            services += 1;
            let ns = s.get("namespace").and_then(|v| v.as_str()).unwrap_or("<ns?>");
            let method = s.get("method").and_then(|v| v.as_str()).unwrap_or("<m?>");
            if let Some(schema) = s.get("input_schema").filter(|v| !v.is_null()) {
                services_input += 1;
                collect_patterns(schema, format!("services.{ns}.{method}.input_schema"), &mut patterns);
                check_types_supported(schema, format!("services.{ns}.{method}.input_schema"), &mut type_errors);
            }
        }
        ...
        for (path, p) in &patterns {
            regex::Regex::new(p).unwrap_or_else(|e| {
                panic!("[{tag}] {path} 声明的 pattern 无法编译（{p}）: {e}")
            });
        }
        assert!(
    ...
    assert!(services_input >= 88, "带契约服务数退化: {services_input} < 88");
