...
func _try_handle(conn: StreamPeerTCP) -> bool:
	...
	# 读取至多 MAX_HEADER_BYTES 的原始数据
	var raw := conn.get_partial(mini(avail, MAX_HEADER_BYTES))
	if raw.size() == 0:
		return false
	...
	var header_part := text.substr(0, header_end)
	...
	if parts.size() < 2:
		...
		return true
	var method := parts[0]
	var raw_target := parts[1]
	var path := raw_target
	var query := ""
	var q_idx := raw_target.find("?")
	if q_idx != -1:
...
func _current_scene_info() -> Dictionary:
	var edt := get_editor_interface() if has_method("get_editor_interface") else null
