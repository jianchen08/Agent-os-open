...
REM
REM  Logging contract (2026-09-15): this script writes no log file
REM  and kernel stdout/stderr are discarded (NUL). The only kernel
REM  log is the kernel's own rotating file layer under logs\
REM  (kernel.log.YYYY-MM-DD, daily rotation, 30 files cap). The
REM  former .kernel_02.log stdout redirect and .kernel_supervisor.log
REM  event log are retired as unbounded append-only surfaces.
REM
...
REM
REM  Usage: run_kernel_supervised.bat <kernel_bin>
