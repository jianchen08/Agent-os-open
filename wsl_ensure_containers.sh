...
if [ "$rc" -ne 0 ]; then
    ...
    if grep -qiE 'cgroup is not empty|failed to create (task|shim task|shim)|container with given ID already exists|task .* already exists' "$COMPOSE_OUT" 2>/dev/null; then
        echo ""
    ...
    echo "[ERROR] 输出尾部: tail -50 $COMPOSE_OUT"
    tail -50 "$COMPOSE_OUT" 2>/dev/null
    rm -f "$COMPOSE_OUT"
    exit "$rc"
fi
