...
> [外部 MCP](plugin-external-mcp.md) 分篇。manifest 全字段见 [docs/guides/plugin-protocol.md](guides/plugin-protocol.md)。
> 想了解内核契约与决策的来龙去脉（traits.rs / manifest 演进 / 契约间依赖）：[契约文件解析教程](contract_files_tutorial.md)。
