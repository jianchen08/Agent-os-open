...
| 工具执行 | `pipeline_tool_core`（Rust 原生）+ 各工具插件（tools/ 27 份 manifest：19 个自研 + 8 个预置外部 MCP 接入，55 个工具声明） |
...
- **分类来源**：内置工具插件（tools/ 19 个自研 + 8 个预置外部 MCP 接入清单）／任意外部 MCP 工具（零代码接入）／用户根自研插件。
