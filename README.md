...
- 🔧 **高度可配置化** —— Agent 不是写死的代码，而是 YAML 数据 + 加载器（批注可以加载动态提示词比如当前时间规则强化不破环缓存命中，还有可以将不同的注入根据使用频率的不同排序提高缓存命中）。改一个提示词不用重启服务（`hot_swap` 支持「快照-替换-健康检查-失败回滚」的热替换）
- 🔄 **自进化闭环** —— 从需求澄清 → 任务派发 → 执行验证 → 评估反馈，每个环节都形成闭环，系统越用越聪明
- 🔌 **插件化管道架构** —— 6 种路由信号（`next_llm` / `next_tool` / `end` / `delegate` / `wait` / `decision`）+ 暂停/恢复机制 + 把每一步决策都变成可观测的状态；插件级错误另设 `ABORT` / `SKIP` / `RETRY` / `FALLBACK` 四种策略框架（当前默认统一 `RETRY`，可按工具维度扩展）
- 🧠 **多层记忆系统** —— 情景记忆（EPISODE）+ 语义记忆（SEMANTIC），按需检索注入。（批注：还有这里的多中注入方式和检索方式写成在下个版本正式上线）
...
- 🔁 **复盘系统** —— 由 `trigger_review` 工具按需触发 `review_agent`，对已结束管道做 LLM 深度复盘，产出经验报告驱动 Agent 配置/插件/工具的迭代（早套按年龄/容量/复盘状态决策的记忆清理机制（实
...
---批注“写上文件夹也就是workspace隔离和docker隔离的，多任务使用worktree机制分叉除worktree目录
