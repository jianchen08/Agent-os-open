...
- 🔌 **一切皆插件** —— 内核只是执行基座（管道解释执行、能力注册、插件装载、存储），LLM、记忆、评估、审批、触发器、通道、主题等一切能力皆插件：改任何业务行为 = 加/改插件或配置，全链热生效、不动内核。
- ⚙️ **高度可配置（插件化的直接结果）** —— Agent 是 YAML 数据：人设、提示词、工具白名单、约束全部可配；动态提示词尾部注入不破坏 prompt cache，改完即生效无需重启。
- 🔄 **自进化闭环** —— 任务执行 → 复盘沉淀经验 → 生成新插件热装载进系统：系统改造自己 = 加插件，不动内核。
- 🔀 **插件化管道** —— 引擎只解释一份管道 YAML：插件读写共享 `state`，声明式路由 DSL 裁决每一步转移（详见[核心亮点 1](#1-插件化管道架构一切皆插件agent-执行的每一处状态都可由你控制)）。
- 🧠 **记忆系统** —— hindsight 记忆插件承载：对话经验自动沉淀（retain）、按需检索（recall）、文档导入，LLM 经 memory 工具读写。
...
- **工具插件**：19 个（41 个内置工具声明，另可零代码接入任意 MCP 外部工具）
- **接入端**：Web 前端（直连内核）
...
> 以下亮点全部建立在"一切皆插件"之上：除引擎与前端外壳，每条能力都由插件承载（能力→插件对照见 [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md)）。

### 1. 插件化管道架构——一切皆插件，Agent 执行的每一处状态都可由你控制

**一切皆插件**：内核只是执行基座（管道解释执行、能力注册表、插件装载、存储），LLM 调用、记忆、评估、审批、触发器、通道、主题等一切能力都由插件承载——这也是"可进化的智能体操作系统"的落地机制：系统自我改造 = 生成新插件热装载生效，改任何业务行为都不动内核；插件崩溃互相隔离，Python 生态即插即用，第三方经 MCP 零代码接入；Web 前端直连内核（HTTP / WebSocket），插件与内核间走 MCP 协议。
...
### 4. 工具系统——契约化设计，开箱即用
统一契约（`input_schema` + `output_schema` + `render` 渲染意图）：内核按 `output_schema` fail-closed 校验结果，前端按 `render` 渲染结果卡片，`tool_ids` 白名单精确控制 LLM 可见面。内置 19 个工具插件、41 个工具（文件、Shell、代码搜索、浏览器、网络、记忆、媒体生成、IDE 集成），任意第三方 MCP 服务零代码接入。
