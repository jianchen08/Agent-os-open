...
### 8. 项目（Project）——复杂长期任务的组织引擎
对于"开发一个 App""写一部网络小说""做一个游戏"这类多阶段、有交付物的大任务，以项目组织任务链：方案规划 → 阶段执行 → 人类审查 → 完成验收闭环。项目 = 用户在界面创建的真实文件夹 + 登记，是任务树的分组锚点（本身不执行）；任务提交携带 `project_id` 挂靠项目，子任务自动继承。

### 9. 工作区隔离与 worktree 机制
