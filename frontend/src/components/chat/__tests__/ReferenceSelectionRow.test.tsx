 ...
 *
 * godot provider 经 '@/services/references' 入口副作用真实注册，只 mock
 * 外部数据通道 selectionBridge——行组件、注册缝、godot 适配器全真实执行：
 * - 有选中 → 渲染 label/chips/清除按钮，点击清除调 clearGodotSelection
 * - 无选中 → 整行不渲染
 * - 订阅推送 → 行随之出现/消失（实时镜像）
...
const bridge = vi.hoisted(() => {
  const state = {
    current: { connected: false, items: [], signature: '' } as GodotSelectionState,
  }
  return {
    state,
    clear: vi.fn(),
    init: vi.fn(),
    listeners: new Set<(s: GodotSelectionState) => void>(),
  }
})

vi.mock('@/services/godot/selectionBridge', () => ({
  getGodotSelection: () => bridge.state.current,
  clearGodotSelection: (...a: unknown[]) => bridge.clear(...a),
  initGodotSelection: (t: string) => bridge.init(t),
  godotPreviewUrl: (i: number, sig: string) => `http://preview/${i}?v=${sig}`,
...
/** 推送快照（模拟插件侧选中变化广播） */
function pushState(next: GodotSelectionState): void {
...
describe('ReferenceSelectionRow — godot provider（经注册缝）', () => {
...
describe('ReferenceSelectionRow — 缝的通用性', () => {
  it('无 getRow 的 provider 不参与渲染；多域行并存', () => {
    ...
    // godot 无选中 + 两假 provider 注册：inject-only 被过滤，fake-row 关闭 → 空
