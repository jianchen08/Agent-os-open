 ...
 * Copyright (c) 2026 DeepSeek. Source: packages/client/ui-primitives/src/ansi.ts
 * Repo pinned at commit 47f943859bef60e4160492346772ded9b24f765a (version 0.1.0-rc.5). Adapted for AgentOS
 * (task_dsh_plugin_adapter 任务 3): verbatim copy (ANSI SGR parser for terminal output).
 */
...
import { ansiToJson } from './ansiToJson.ts'

/* eslint-disable no-control-regex -- ANSI 转义解析器：正则必须匹配控制字符（DSH 原样移植） */
...
export function parseAnsiLines(text: string): AnsiLine[] {
  ...
  for (const chunk of ansiToJson(sanitize(text))) {
