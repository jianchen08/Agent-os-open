...
export function MediaTimelineView({
  ...
  return (
    <div className="media-timeline-view flex h-full flex-col" data-testid="media-timeline-view">
      {/* 媒体播放器 */}
      <div className={`relative flex-1 ${mediaType === 'video' ? 'bg-[var(--media-canvas)]' : 'bg-muted'}`}>
        {mediaType === 'video' ? (
          <video
        ...
        <div className="flex items-center gap-2 bg-background border-t border-border px-3 py-2">
          <button
            onClick={togglePlay}
            className="flex h-8 w-8 items-center justify-center rounded-full bg-status-info text-status-info-foreground"
      ...
      <div className="border-t border-border bg-background px-3 py-2">
        ...
        >
          {/* 播放进度 */}
          <div
            className="absolute left-0 top-0 h-full rounded bg-status-info/30"
          ...
          {videoAnnotations.map((a, idx) => {
            ...
            return (
              ...
              >
                <div className="mt-0.5 flex h-4 w-4 items-center justify-center rounded-full bg-status-warning text-[8px] font-bold text-status-warning-foreground">
                  {idx + 1}
                </div>
              </div>
            )
        ...
        {videoAnnotations.length > 0 && (
          <div className="mt-2 space-y-1" data-testid="annotation-list">
            {videoAnnotations.map((a, idx) => (
              ...
              >
                <span className="flex h-4 w-4 shrink-0 items-center justify-center rounded-full bg-status-warning text-[8px] font-bold text-status-warning-foreground">
                  {idx + 1}
                </span>
