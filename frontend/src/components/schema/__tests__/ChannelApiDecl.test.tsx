...
describe('trigger_setup_tool 触发器声明（B1）', () => {
  it('声明存在：triggers_table（列+rowActions+watch）与 triggers_create（form 契约字段）', () => {
    ...
    const formProps = form?.props as {
      eventName?: string
      fields?: Array<{ name: string; datasourceUri?: string; required?: boolean }>
      endpoint?: string
    } | undefined
  ...
  it('创建表单：填消息提交（类型预选首项）→ 切类型再提交（body=工具入参契约）', async () => {
    ...
    // 类型 select 值缺省时落首选项（RJSF/antd 既有语义）——填消息即可提交
    ...
    expect(body).toMatchObject({ trigger_type: 'delay', message: '检查任务状态' })
    ...
    expect(body).toMatchObject({ trigger_type: 'interval', message: '检查任务状态' })
    vi.unstubAllGlobals()
  })
