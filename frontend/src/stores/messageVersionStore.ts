...
import type { Message, MessageVersion, VersionDiff, MessageToolCall } from '@/types/models'
...
interface MessageVersionState {
   ...
   * @returns 版本差异
   */
  compareVersions: (messageId: string, version1: number, version2: number) => VersionDiff | null
 ...
 * 消息版本管理 Store
 */
export const useMessageVersionStore = create<MessageVersionState>((set, get) => ({
  versions: {},
  currentVersions: {},
  ...
  restoreVersion: (messageId: string, version: number) => {
    ...
    if (!targetVersion) {
      console.warn(`[messageVersionStore] 消息 ${messageId} 没有版本 ${version}`)
      return null
    }
   ...
   * 比较两个版本
   */
  compareVersions: (messageId: string, version1: number, version2: number) => {
    const state = get()
    const versions = state.versions[messageId]
