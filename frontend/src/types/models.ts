...
export interface MessageVersion {
  ...
  isCurrent: boolean
  /** 版本快照的完整消息数据（用于恢复） */
  messageSnapshot: Omit<Message, 'versions' | 'currentVersion'>
}

/**
 * 扩展的 Message 接口 - 支持版本管理
 */
export interface ExtendedMessage extends Message {
  ...
  /** 是否正在重试 */
  isRetrying?: boolean
  /** 重试范围（all | failed_tools | specific_tool） */
  retryScope?: RetryScope
  /** 重试的目标工具ID（retryScope='specific_tool' 时） */
  retryTargetToolId?: string
}
 ...
 * 扩展的 Message 接口（支持分层）
 */
export interface LayeredMessage extends Message {
  /** 所属 Agent 层级 */
  ...
  /** Agent 路径（从主 Agent 到当前 Agent 的路径） */
  agentPath?: string[]
