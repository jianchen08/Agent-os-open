 ...
 * 修复：longTermTasks.ts 全部端点迁移到 /ext/task_service/tasks（task_service 插件自持该域）。
 *
 * 本测试断言可观察行为：所有长期任务 API 请求必须使用 /ext/task_service/tasks 前缀，
...
describe('longTermTasks API 端点路径（4c 迁移：/ext/task_service/tasks）', () => {
  ...
  it('fetchLongTermTasks 请求 /ext/task_service/tasks 而非 /api/v1/tasks', async () => {
    await longTermTaskApi.fetchLongTermTasks({ page: 1, limit: 100 })

    expect(getMock).toHaveBeenCalledTimes(1)
    const [url] = getMock.mock.calls[0]
    expect(url).toContain('/ext/task_service/tasks')
  ...
  it('toggleAutoExecute 请求 /ext/task_service/tasks/{id}', async () => {
    await longTermTaskApi.toggleAutoExecute('t1', true)

    expect(getMock).toHaveBeenCalledTimes(1)
    expect(getMock.mock.calls[0][0]).toBe('/ext/task_service/tasks/t1')
    expect(patchMock).toHaveBeenCalledTimes(1)
    expect(patchMock.mock.calls[0][0]).toBe('/ext/task_service/tasks/t1')
  })

  it('cancelLongTermTask 请求 POST /ext/task_service/tasks/{id}/cancel', async () => {
  ...
  it('pause/resume 请求 PATCH /ext/task_service/tasks/{id}', async () => {
    ...
    expect(patchMock.mock.calls[0][0]).toBe('/ext/task_service/tasks/t1')
    expect(patchMock.mock.calls[1][0]).toBe('/ext/task_service/tasks/t1')
  })
})
