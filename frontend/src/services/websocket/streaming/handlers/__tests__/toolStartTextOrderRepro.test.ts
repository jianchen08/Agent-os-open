 ...
 *
 * 根因假设：
 *  handleStreamChunk 只把文本 chunk 缓冲到 RAF（不立即创建 text part），
 *  而 handleToolStart 同步执行。当 tool_start 在 RAF flush 之前到达：
 *    1. text part 尚未创建（chunk 仍在 buffer）
 ...
 *
 * 本测试用 fake timers 精确控制 RAF 时序，证实该假设。
 */
...
describe('工具调用与文本块顺序：tool_start 与 buffered text chunk 的时序竞争', () => {
  ...
  it('复现：text chunk buffered 未 flush 时 tool_start 到达 → 文本被错误劈到工具之后', async () => {
    handleStreamStart(makeEvent('stream_start', { sequence: 1 }))

    // 正文文本 chunk（缓冲到 RAF，不立即创建 text part）
    handleStreamChunk(makeEvent('stream_chunk', { content: '工具调用前的正文', sequence: 2 }))
    // 【关键】不推进 RAF —— 镜像"tool_start 在同一帧内紧随 chunk 到达"

    // tool_start 同步到达：此时 text part 尚未创建（chunk 仍在 buffer）
    handleToolStart(makeEvent('tool_start', {
      call_id: 'tc-1', tool_name: 'search', args: {}, sequence: 3,
    }))

    // 现在推进 RAF，buffered chunk 才被 flush
    ...
    // ❌ 实际（bug）：[tool_call, text]（正文被劈到工具后）
  ...
  it('完整多轮：思考1→正文1→工具→思考2→正文2（不应出现思考-工具-文本-文本-思考）', async () => {
    ...
    handleStreamChunk(makeEvent('stream_chunk', { content: '正文1', sequence: 2 }))
    // 不推进 RAF（镜像真实主线程时序）
    ...
    // 第二轮：思考2 → 正文2
    handleThinkingStart(makeEvent('thinking_start', { sequence: 4 }))
    handleThinkingChunk(makeEvent('thinking_chunk', { content: '第二轮思考', sequence: 4 }))
    handleThinkingEnd(makeEvent('thinking_end', { duration_ms: 80 }))
    handleStreamChunk(makeEvent('stream_chunk', { content: '正文2', sequence: 5 }))
