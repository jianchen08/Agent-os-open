 ...
 *
 * 验证：
 * 1. 正常 payload → 按 pipeline 分桶写入单轮 token + 累计 cumulative
 * 2. total_tokens 为 0（tool_execute 残留）→ 不覆盖已有值
 * 3. 缺 pipeline_id → 跳过
 * 4. cumulative 缺失时保留上一轮累计值（避免统计闪烁）
 */
...
describe('handleCostUpdate', () => {
  ...
  it('cumulative 字段：写入命中/未命中/输出三维度累计值', () => {
    handleCostUpdate({
      type: 'cost_update',
      data: {
        pipeline_id: 'pipe-cum',
        total_tokens: 1500,
        input_tokens: 1200,
        output_tokens: 300,
        cumulative: {
    ...
    const usage = useContextUsageStore.getState().getUsage('pipe-cum')
    expect(usage?.cumulative).toBeDefined()
    expect(usage?.cumulative?.inputTokens).toBe(5000)
    expect(usage?.cumulative?.outputTokens).toBe(800)
    expect(usage?.cumulative?.cachedTokens).toBe(3000)
    expect(usage?.cumulative?.missedTokens).toBe(2000)
    expect(usage?.cumulative?.totalTokens).toBe(5800)
  })

  it('cumulative 缺失时保留上一轮累计值，避免统计闪烁', () => {
    // 第一轮：带 cumulative
    handleCostUpdate({
      type: 'cost_update',
      data: {
        pipeline_id: 'pipe-keep',
        total_tokens: 1000,
        input_tokens: 800,
        output_tokens: 200,
        cumulative: {
    ...
    const usage = useContextUsageStore.getState().getUsage('pipe-keep')
    // 单轮值应更新为第二轮
    expect(usage?.totalTokens).toBe(500)
    // 累计值应保留第一轮的 1000，不被 undefined 清空
    expect(usage?.cumulative?.totalTokens).toBe(1000)
  })
