...
export class MessageFormatDetector {
   ...
   * @returns 是否为统一输入消息格式
   */
  static isUnifiedIncoming(message: unknown): message is UnifiedIncomingMessage {
    ...
    return (
      ...
      ['user_input', 'command', 'system_event'].includes(msg.type) &&
   ...
   * @returns 是否为统一流式事件格式
   */
  static isUnifiedStreamEvent(message: unknown): message is UnifiedStreamEvent {
    if (!message || typeof message !== 'object') {
      return false
    }
    ...
    // 检查必需字段
    const validEventTypes: UnifiedEventType[] = [
      'stream.chunk',
      'stream.end',
      'stream.error',
      'tool.start',
    ...
    return (
      ...
      validEventTypes.includes(msg.event_type as UnifiedEventType) &&
      ...
      typeof (msg.metadata as Record<string, unknown>).timestamp === 'string'
    )
  }
}
