/**
 * 渲染路由器（render 意图路由层：声明路由 + 数据形状路由）。
 *
 * 工具卡片渲染采用「双路由」：
 ...
 *
 * **card 是词汇表，不是组件绑定**：本模块只把数据映射成 ActivityDetailBlock
 * 的原生字段（contentType + 结构化字段），组件由 ActivityCard 按 contentType
 * 统一渲染——插件声明什么形态就渲染什么形态，前端不为任何插件写专属组件。
 *
 * 本模块全部是**纯函数**（除注册表装载/查询外无副作用）——数据映射
 * （灵汐 toolCall/resultData → 渲染字段）字段绑定可经 render.bindings
...
export function renderIntentToBlocks(
  ...
  // payload → 原生块字段（各 contentType 由 ActivityCard 统一渲染）；
  // 卡片词汇表与渲染组件解耦：新增卡片形态 = 本表加一条映射，前端组件零改动。
  switch (intent.card) {
    case 'image':
    case 'file': {
    ...
    case 'diff': {
      // 多文件 diffs → 每文件一块（块标题 = 文件路径）；单文件对 → 一块
      const diffs = Array.isArray(payload.diffs)
        ? payload.diffs as { path: string; oldText: string | null; newText: string }[]
        : []
      return diffs.map((d, i) => ({
        id: `diff-${i}`,
        label: intent.title ?? (d.path !== '' ? d.path : '差异对比'),
        content: '',
    ...
    case 'read': {
      const lines = Array.isArray(payload.lines)
        ? payload.lines as { number: number; text: string }[]
        : []
      const totalLines = typeof payload.totalLines === 'number' ? payload.totalLines : lines.length
      return [{
        label: (typeof payload.label === 'string' && payload.label !== '' ? payload.label : intent.title) ?? '文件内容',
        content: '',
        contentType: 'read',
        read: {
          lines,
          totalLines,
          ...(typeof payload.lang === 'string' ? { lang: payload.lang } : {}),
        },
      }]
    }
    case 'terminal': {
      return [{
        label: (typeof payload.command === 'string' ? payload.command : intent.title) ?? '命令',
        content: '',
      ...
      if (payload.kind === 'paths') {
        ...
        return [{
          ...
          search: {
            ...
            total: typeof payload.total === 'number' ? payload.total : paths.length,
          },
        }]
      }
      const files = Array.isArray(payload.files)
        ? payload.files as { path: string; matches: { lineNumber: number; line: string }[] }[]
        : []
      return [{
        label: intent.title ?? '搜索结果',
        content: '',
    ...
    case 'web': {
      ...
      const sources = Array.isArray(payload.sources)
        ? payload.sources as { url: string; title?: string; snippet?: string; publishedAt?: string }[]
        : []
      return [{
        label: intent.title ?? '联网搜索',
        content: '',
        contentType: 'web',
        web: {
          kind: 'search',
