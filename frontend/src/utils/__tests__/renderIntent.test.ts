...
describe('applyDataDrivenIntent（数据路由增强入口）', () => {
  ...
  it('无声明 + 数据有形状 → 按形状路由（diff 数据 → diff 块）', () => {
...
describe('renderIntentToBlocks / applyRenderIntent（级联集成）', () => {
  ...
  it('产出原生 terminal 块，字段直取', () => {
    const blocks = renderIntentToBlocks(
      { card: 'terminal' },
      ctx({ command: 'pwd' }, { output: '/tmp', exit_code: 0 }),
    )
    expect(blocks).toHaveLength(1)
    expect(blocks[0].contentType).toBe('terminal')
    expect(blocks[0].terminal).toMatchObject({ command: 'pwd', output: '/tmp' })
  })
  ...
  it('applyRenderIntent：声明 + 数据可映射 → details 被替换', () => {
    ...
    expect(out?.details?.[0].contentType).toBe('read')
    // 原 details 被声明渲染替换
