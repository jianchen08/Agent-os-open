...
class TaskExecutor:
    ...
    async def start(self):
        ...
        logger.info("[TaskExecutor] 已订阅 task.execution_requested 事件")
    ...
    async def stop(self):
        ...
        if self._subscription_id and self._event_bus:
            ...
            self._subscription_id = None
            self._event_bus = None
            logger.info("[TaskExecutor] 已取消订阅")
    ...
    async def _on_execution_requested(self, event):
        ...
        if not task_id:
            logger.warning("[TaskExecutor] 收到无效的执行请求事件 | 缺少 task_id")
            return
        ...
        if source == "recovery":
            ...
            logger.info(
                f"[TaskExecutor] 恢复任务，清除执行追踪状态 | task_id={task_id}"
            )

        if task_id in self._executing_tasks:
            logger.warning(
                f"[TaskExecutor] 任务已在执行中，跳过重复请求 | task_id={task_id}"
            )
            return
        ...
        if fail_count >= self.MAX_RETRIES:
            logger.error(
                f"[TaskExecutor] 任务重试次数超限({fail_count}/{self.MAX_RETRIES})，放弃执行 | task_id={task_id}"
            )
            return

        logger.info(f"[TaskExecutor] 收到执行请求 | task_id={task_id} | source={source}")
    ...
    async def execute_task_with_result(self, task_id: str) -> dict:
        ...
        """
        logger.info(f"[TaskExecutor] ===== 开始执行任务 ===== | task_id={task_id}")
        ...
        try:
            ...
            self._failed_tasks[task_id] = self._failed_tasks.get(task_id, 0) + 1
            logger.exception(
                f"[TaskExecutor] 任务执行异常 | task_id={task_id} | error={str(e)}"
            )
            return {
                "success": False,
    ...
    async def _execute_task_internal(
        ...
        if not task:
            logger.error(f"[TaskExecutor] 任务不存在 | task_id={task_id}")
            return {
                "success": False,
                "error": "任务不存在",
                "task_id": task_id,
        ...
        if task.status == "in_progress":
            logger.info(
                f"[TaskExecutor] 任务状态为 in_progress，可能是恢复执行 | task_id={task_id}"
            )

        try:
            # 3. 更新任务状态为 in_progress
            await self._update_task_status(task_id, "in_progress", session)
            logger.info(
                f"[TaskExecutor] 任务状态已更新为 in_progress | task_id={task_id}"
            )
    ...
    async def _load_agent_config(
        ...
        if not agent_model:
            stmt = select(AgentConfigModel).where(
                AgentConfigModel.config_id == agent_id
            ).limit(1)
            result = await session.execute(stmt)
            agent_model = result.scalar_one_or_none()
            if agent_model:
                logger.info(
                    f"[TaskExecutor] 通过 config_id 找到 Agent | "
        ...
        if not agent_model:
            logger.debug(
                f"[TaskExecutor] 通过 ID/config_id 未找到 Agent，尝试通过名称查找 | "
                f"agent_id={agent_id}"
            )
            stmt = select(AgentConfigModel).where(AgentConfigModel.name == agent_id).limit(1)
            result = await session.execute(stmt)
            agent_model = result.scalar_one_or_none()
            if agent_model:
                logger.info(
                    f"[TaskExecutor] 通过名称找到 Agent | "
    ...
    def _build_user_input(
        ...
        # 添加执行要求
        parts.append("")
        agent_level = agent_config.level if agent_config else 1
        ...
        if agent_level == 3:
            ...
            if use_todo:
                ...
                parts.append(self._get_simple_execution_guide(task))
...
**执行流程：**
1. **制定计划** - 首先列出执行步骤（每步包括目标和产出）
2. **逐步执行** - 按计划执行，每完成一步标记 ✅
3. **完成报告** - 总结产出和结果

**约束：**
- 不能创建子任务或调用其他执行者
- 只能使用工具完成任务
- 必须在回复最开始生成执行计划{criteria_str}
...
**完成后调用：** task_evaluate(action='auto_complete', task_id='{task.id}')"""
    ...
    def _get_l2_execution_guide(self, task: Task) -> str:
        ...
        if needs_preparation:
            ...
            return self._get_simple_execution_guide(task)
...
3. 汇总结果并完成任务"""
    ...
    def _get_simple_execution_guide(self, task: Task) -> str:
        """生成简单执行指引（简单任务）"""
        return f"""【执行方式】
本任务较简单，按以下流程执行：

1. 先根据任务的实际情况判断是否需要分解：
   - 如果任务涉及多个独立的功能模块或步骤，应该分解成多个子任务
   - 如果任务是单一明确的操作，可以直接提交
   - 分解时要考虑任务的逻辑依赖关系和执行顺序

2. 将任务（或分解后的子任务）提交给相应的 L3 Agent 执行：
   - 根据任务类型选择合适的 L3 Agent（如 backend_agent、frontend_agent、database_agent 等）
   - 不要提交给通用执行者，而是提交给专门负责该领域的 Agent
   - 如果有多个子任务，按照依赖关系依次或并行提交

3. 完成后调用 task_evaluate(action='auto_complete', task_id='{task.id}') 提交评估"""
    ...
    async def _execute_agent(
        ...
        logger.debug(
            f"[TaskExecutor._execute_agent] 已注册内置工具: {registered_tools}"
        )
    ...
    async def _send_execution_start(
        ...
        try:
            ...
            logger.warning(
                f"[TaskExecutor] 发送执行开始事件失败 | task_id={task_id} | error={e}"
            )
    ...
    async def _send_execution_done(
        ...
        try:
            ...
            logger.warning(
                f"[TaskExecutor] 发送执行完成事件失败 | task_id={task_id} | error={e}"
            )
