...
from src.agents.types import AgentConfig, AgentResult
...
class AgentExecutor:
    ...
    async def execute_task(self, task: TaskRequest) -> dict[str, Any]:
        ...
        from src.api.websocket.message_bus import get_message_bus, SourceType
        ...
        try:
            ...
            # 执行任务
            result: AgentResult = await agent_loop.run(task.prompt)
    ...
    async def cancel_task(self, task_id: str) -> bool:
        ...
        if task_id in self._running_agents:
            agent_loop = self._running_agents[task_id]
