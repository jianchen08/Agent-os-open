...
2. 恢复 running 任务 - 重新触发执行
3. 恢复依赖等待状态 - 恢复到 TaskOrchestrator

设计文档: docs/design/task-recovery-mechanism.md

迁移说明：
- 原位置: src/tasks/recovery_orchestrator.py
- 新位置: src/orchestration/recovery/recovery_orchestrator.py
- 迁移时间: 2026-02-27
- 迁移原因: RecoveryOrchestrator 编排跨模块恢复，应归属 orchestration 模块
"""
...
class RecoveryConfig:
    ...
    Attributes:
        ...
        restore_running: 是否恢复 running 任务
...
class RecoveryResult:
    ...
    Attributes:
        ...
        running_restored: 恢复的 running 任务数量
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "running_restored": self.running_restored,
...
class TaskRecoveryOrchestrator:
    ...
    async def restore(self) -> RecoveryResult:
        ...
        try:
            ...
            if self.config.restore_running:
                result.running_restored = await self._restore_running_tasks()
        ...
        logger.info(
            ...
            f"running={result.running_restored} | "
        ...
        async with self.session_factory() as session:
            ...
            for task in tasks:
                try:
                    ...
                    from src.orchestration import schedule

                    schedule_result = await schedule(task.id)
                    if schedule_result.get("success"):
                        logger.info(
                            f"[TaskRecoveryOrchestrator] 已恢复并启动 pending 任务 | task_id={task.id}"
                        )
                    else:
                        logger.warning(
                            f"[TaskRecoveryOrchestrator] 恢复 pending 任务失败 | "
                            f"task_id={task.id} | error={schedule_result.get('error')}"
                        )
    ...
    async def _restore_running_tasks(self) -> int:
        """
        恢复 running 状态任务
        ...
        async with self.session_factory() as session:
            ...
            logger.info(
                f"[TaskRecoveryOrchestrator] 发现 {len(tasks)} 个 running 任务待恢复"
            )
            ...
            for task in tasks:
                try:
                    ...
                    if schedule_result.get("success"):
                        logger.info(
                            f"[TaskRecoveryOrchestrator] 已恢复并启动 running 任务 | task_id={task.id}"
                        )
                    else:
                        logger.warning(
                            f"[TaskRecoveryOrchestrator] 恢复 running 任务失败 | "
                            f"task_id={task.id} | error={schedule_result.get('error')}"
                        )

                    await self._mark_recovered(session, task.id, "running")
                    restored += 1
