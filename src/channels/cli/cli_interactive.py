...
class CLIInteractiveMixin:
        ...
        while True:
            ...
            if self._pipeline_task is not None and self._pipeline_task.done():
                ...
                try:
                    ...
                    logger.exception(
                        "Error handling pipeline result, continuing REPL loop"
                    )
            ...
            if cli_notifier and cli_notifier.has_pending():
                ...
                await self._run_sub_conversation_safe(
                    console, cli_notifier, human_svc, "top"
                )
            ...
            if _pipeline_was_running:
                if await self._repl_wait_for_output(cli_notifier):
                    continue
            ...
            try:
                initial_state, pipeline_done = (
                    await self._wait_for_next_event(cli_notifier, console)
                )
                ...
                logger.warning(
                    ...
                    "(iter=%d): %s",
                    _repl_iteration, _wait_exc, exc_info=True,
                )
                continue
            ...
            if initial_state.get("should_stop"):
                _exit_reason = await self._repl_handle_should_stop(
                    console, _repl_iteration
                )
                if _exit_reason:
                    break
                continue
            ...
            if initial_state.get("_is_empty"):
                if cli_notifier and cli_notifier.has_pending():
                    human_svc = self._services.get("human_interaction_service")
                    await self._run_sub_conversation_safe(
                        console, cli_notifier, human_svc, "empty"
                    )
                    self._input_adapter.drain_stdin()
                continue

            # --- 斜杠命令处理 ---
            cmd_handled, _exit_reason = (
                await self._repl_handle_slash_commands(
                    initial_state, console,
                    conversation_history, session, session_svc,
                )
            )
            if _exit_reason:
                break
            ...
            if (
                ...
                if await self._repl_handle_pipeline_busy(
                    initial_state, console, cli_notifier
                ):
                    continue
    ...
    async def _repl_init_session(
            ...
            if exec_storage:
                try:
                    ...
                    if prev_records:
                        ...
                        from infrastructure.task_worker import (
                            _reconstruct_tool_calls,
                        )
                        _reconstruct_tool_calls(conversation_history)
                        restored = True
                        logger.info(
                            "Restored %d messages from pipeline records "
                            "(pipeline=%s)",
                    ...
                    logger.debug(
                        "Failed to restore from pipeline records: %s", exc
                    )
        logger.info(
            "[STARTUP] conversation restore (%d msgs): %.2fs",
            len(conversation_history), _time.monotonic() - _run_t4,
        )
        logger.info(
        ...
        if restored:
            console.print(
                f"[dim]已恢复上次会话 "
                f"({len(conversation_history)} 条消息)，"
    ...
    async def _repl_handle_should_stop(
        self, console: Any, _repl_iteration: int,
    ) -> str:
        """处理退出信号。管道运行中则阻止退出。
        ...
        if _pipeline_still_running:
            logger.warning(
                ...
                "(iter=%d) — ignoring", _repl_iteration,
            )
            return ""
        ...
        try:
            ...
            if all_tasks:
                ...
                for t in all_tasks:
                    ...
                    tstatus = (
                        t.status if hasattr(t, "status")
                        else t.get("status", "?")
                    )
                    tstatus_str = (
                        tstatus.value if hasattr(tstatus, "value")
                        else str(tstatus)
                    )
                    ttitle = (
                    ...
                    icon = (
                        ...
                        else "❌" if tstatus_str == "failed"
                        else "🔄"
                    )
                    console.print(
                        f"  {icon} {tid[:12]} | {tstatus_str} | {ttitle}"
                    )
        except Exception as exc:
    ...
    async def _repl_handle_slash_commands(
        ...
        # 旧版斜杠命令兼容
        if (
            hasattr(self._input_adapter, "is_slash_command")
            and self._input_adapter.is_slash_command(initial_state)
        ):
    ...
    async def _repl_handle_pipeline_busy(
        ...
        if cli_notifier and cli_notifier.has_pending():
            human_svc = self._services.get("human_interaction_service")
            await self._run_sub_conversation_safe(
                console, cli_notifier, human_svc, "busy"
            )
            return True
    ...
    def _repl_start_pipeline(
        ...
        self._output_adapter.update_status_bar(
            ...
            pipeline_max_iterations=(
                self._engine.max_iterations if self._engine else 0
            ),
