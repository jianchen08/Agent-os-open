...
class LSPTools:
    ...
    async def _lsp_definition(self, **kwargs) -> ToolExecutionResult:
        """
        执行跳转到定义
        ...
        try:
            from src.lsp.file_jump import FileJumpProtocol
            from src.lsp.gateway import get_lsp_gateway
            from src.lsp.types import Position
            ...
            if success:
                return create_success_result(
                    output=f"已跳转到定义: {first_location.uri}",
                    metadata={
    ...
    async def _lsp_references(self, **kwargs) -> ToolResult:
        """
        执行查找引用
    ...
    async def _lsp_diagnostics(self, **kwargs) -> ToolResult:
        """
        执行获取诊断
    ...
    async def _file_jump(self, **kwargs) -> ToolResult:
        """
        执行文件跳转
        ...
        try:
            ...
            if success:
                return create_success_result(
                    output=f"已打开文件: {str(validated_path)}",
                    metadata={
                        "file_path": str(validated_path),
                        "line": line,
                        "character": character,
