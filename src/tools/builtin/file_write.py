...
提供文件的创建、全量写入和局部修改功能。
与 FileReadTool 分工明确：
- FileReadTool: 负责文件的读取、目录列表、文件搜索（读操作）
- FileWriteTool: 负责文件的创建、写入、编辑（写操作）
...
class FileWriteTool(BuiltinTool):
    ...
    async def _write(self, inputs: dict[str, Any]) -> ToolResult:
        ...
        行为：
        - 无行号参数: 全量覆盖写入（文件不存在则创建）
        - 指定 start_line 无 end_line: 替换单行
        - 指定 start_line 和 end_line: 替换行范围（包含边界）
    ...
    async def _delete_lines(self, inputs: dict[str, Any]) -> ToolResult:
        ...
        删除从 start_line 到 end_line 的行范围（包含边界）
