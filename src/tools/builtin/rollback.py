...
class CheckpointTool:
    ...
    def get_tool_definition() -> Tool:
        ...
        return Tool(
            ...
            description="为当前任务创建检查点，用于后续回滚。"
            "适用场景：关键操作前保存任务状态、执行可能出错的操作前、创建可恢复的快照点。"
            "不适用场景：任务尚未开始时、操作非常简单且不会出错时、已在上一步创建检查点时。"
            "注意事项：检查点会占用存储空间，不要过度创建；检查点只保存任务状态，不保存外部系统状态；"
            "建议在每个重要操作步骤前创建检查点；检查点名称应具有描述性，便于后续识别。",
            input_schema={
                "type": "object",
                "properties": {
                    "task_id": {
                        "type": "string",
                        "description": "任务ID，用于标识要创建检查点的任务",
                    },
                    "name": {
                        "type": "string",
                        "description": "检查点名称（可选），用于标识和描述此检查点的用途",
                    },
                    "description": {
                        "type": "string",
                        "description": "检查点描述（可选），提供更详细的检查点说明信息",
                    },
                },
                "required": ["task_id"],
            },
