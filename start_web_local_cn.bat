...
:: ===========================================================================
:: 5. 启动后端（端口 8989，内存模式降级，无需 Redis）
:: ===========================================================================
echo [INFO] 启动后端（端口 8989，不依赖 Redis，自动内存模式）...
start "Agent OS Backend (local)" /D "%cd%" cmd /k "set PYTHONPATH=src&& set BACKEND_PORT=8989&& "%PYEXE%" -m channels.websocket.app_factory"

:: ===========================================================================
:: 6. 启动前端（vite dev server，端口 5290，proxy 到 8989）
:: ===========================================================================
echo [INFO] 启动前端（vite dev server，端口 5290）...
...
echo   启动完成
echo ========================================
echo   后端: http://localhost:8989
echo   前端: http://localhost:5290
