...
def main() -> int:
    ...
    if pipelines:
        ...
        for r in cur.execute(f"SELECT run_id FROM runs WHERE pipeline_id IN ({pid_ph})", pipelines).fetchall():
            if r["run_id"] and r["run_id"] not in run_ids:
                run_ids.append(r["run_id"])
    # 无管道归属的老 run（pipeline_id IS NULL 且不属于保留管道）
    for r in cur.execute('SELECT run_id FROM runs WHERE pipeline_id IS NULL OR pipeline_id = ""').fetchall():
        if r["run_id"] and r["run_id"] not in run_ids:
            run_ids.append(r["run_id"])
    ...
    if pipelines:
        n = cur.execute(
            f"SELECT COUNT(*) FROM execution_records WHERE pipeline_run_id IN ({pid_ph})", pipelines
        ).fetchone()[0]
    ...
    try:
        ...
        if pipelines:
            ...
            cur.execute(f"DELETE FROM execution_records WHERE pipeline_run_id IN ({pid_ph})", pipelines)
        if run_ids:
            rid_ph = ",".join("?" * len(run_ids))
        ...
        if threads:
            th_ph = ",".join("?" * len(threads))
            cur.execute(f"DELETE FROM pipeline_sessions WHERE thread_id IN ({th_ph})", threads)
            cur.execute(f"DELETE FROM sessions WHERE thread_id IN ({th_ph})", threads)
