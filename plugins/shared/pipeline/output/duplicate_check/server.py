...
from plugin import DuplicateCheckPlugin  # noqa: E402

logger = logging.getLogger(__name__)
plugin = AgentOSPlugin("duplicate_check_pipeline")

_instance: DuplicateCheckPlugin | None = None


@plugin.on_load
async def _on_load(params: dict) -> None:
    """Initialize duplicate_check plugin."""
    global _instance
    config = plugin.get_config()
    _instance = DuplicateCheckPlugin(config=config)


@plugin.on_unload
async def _on_unload(params: dict) -> None:
    """Cleanup duplicate_check plugin."""
    global _instance
    _instance = None


@plugin.tool(
    name="duplicate_check.execute",
    schema={
        "type": "object",
        "properties": {
    ...
    description="Execute Duplicate Check pipeline plugin",
)
async def execute(state: dict, config: dict | None = None) -> dict:
    """Execute the duplicate_check pipeline plugin.
