#!/usr/bin/env python3
"""event_callback output pipeline plugin MCP 服务端——纯接口适配层。

老代码从 src/plugins/shared/output/event_callback/plugin.py 原封不动复制到本目录，
...
from plugin import EventCallbackPlugin  # noqa: E402

logger = logging.getLogger(__name__)
plugin = AgentOSPlugin("event_callback_pipeline")

_instance: EventCallbackPlugin | None = None


@plugin.on_load
async def _on_load(params: dict) -> None:
    """Initialize event_callback plugin."""
    global _instance
    config = plugin.get_config()
    _instance = EventCallbackPlugin(config=config)


@plugin.on_unload
async def _on_unload(params: dict) -> None:
    """Cleanup event_callback plugin."""
    global _instance
    _instance = None


@plugin.tool(
    name="event_callback.execute",
    schema={
        "type": "object",
        "properties": {
    ...
    description="Execute Event Callback pipeline plugin",
)
async def execute(state: dict, config: dict | None = None) -> dict:
    """Execute the event_callback pipeline plugin.
