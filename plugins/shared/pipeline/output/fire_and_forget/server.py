#!/usr/bin/env python3
"""fire_and_forget output pipeline plugin MCP 服务端——纯接口适配层。

老代码从 src/plugins/shared/output/fire_and_forget/plugin.py 原封不动复制到本目录，
...
from plugin import FireAndForgetPlugin  # noqa: E402

logger = logging.getLogger(__name__)
plugin = AgentOSPlugin("fire_and_forget_pipeline")

_instance: FireAndForgetPlugin | None = None


@plugin.on_load
async def _on_load(params: dict) -> None:
    """Initialize fire_and_forget plugin."""
    global _instance
    config = plugin.get_config()
    _instance = FireAndForgetPlugin(config=config)


@plugin.on_unload
async def _on_unload(params: dict) -> None:
    """Cleanup fire_and_forget plugin."""
    global _instance
    _instance = None


@plugin.tool(
    name="fire_and_forget.execute",
    schema={
        "type": "object",
        "properties": {
    ...
    description="Execute Fire And Forget pipeline plugin",
)
async def execute(state: dict, config: dict | None = None) -> dict:
    """Execute the fire_and_forget pipeline plugin.
