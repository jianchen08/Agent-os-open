#!/usr/bin/env python3
"""tool_progress output pipeline plugin MCP 服务端——纯接口适配层。

老代码从 src/plugins/shared/output/tool_progress/plugin.py 原封不动复制到本目录，
...
from plugin import ToolProgressReporter  # noqa: E402

logger = logging.getLogger(__name__)
plugin = AgentOSPlugin("tool_progress_pipeline")

_instance: ToolProgressReporter | None = None


@plugin.on_load
async def _on_load(params: dict) -> None:
    """Initialize tool_progress plugin."""
    global _instance
    config = plugin.get_config()
    _instance = ToolProgressReporter(config=config)


@plugin.on_unload
async def _on_unload(params: dict) -> None:
    """Cleanup tool_progress plugin."""
    global _instance
    _instance = None


@plugin.tool(
    name="tool_progress.execute",
    schema={
        "type": "object",
        "properties": {
    ...
    description="Execute Tool Progress pipeline plugin",
)
async def execute(state: dict, config: dict | None = None) -> dict:
    """Execute the tool_progress pipeline plugin.
