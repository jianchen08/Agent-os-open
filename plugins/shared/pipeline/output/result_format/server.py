#!/usr/bin/env python3
"""result_format output pipeline plugin MCP 服务端——纯接口适配层。

老代码从 src/plugins/shared/output/result_format/plugin.py 原封不动复制到本目录，
...
from plugin import ResultFormatPlugin  # noqa: E402

logger = logging.getLogger(__name__)
plugin = AgentOSPlugin("result_format_pipeline")

_instance: ResultFormatPlugin | None = None


@plugin.on_load
async def _on_load(params: dict) -> None:
    """Initialize result_format plugin."""
    global _instance
    config = plugin.get_config()
    _instance = ResultFormatPlugin(config=config)


@plugin.on_unload
async def _on_unload(params: dict) -> None:
    """Cleanup result_format plugin."""
    global _instance
    _instance = None


@plugin.tool(
    name="result_format.execute",
    schema={
        "type": "object",
        "properties": {
    ...
    description="Execute Result Format pipeline plugin",
)
async def execute(state: dict, config: dict | None = None) -> dict:
    """Execute the result_format pipeline plugin.
