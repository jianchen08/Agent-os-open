#!/usr/bin/env python3
"""multimodal_postprocessor output pipeline plugin MCP 服务端——纯接口适配层。

老代码从 src/plugins/shared/output/multimodal_postprocessor/plugin.py 原封不动复制到本目录，
...
from plugin import MultimodalPostprocessor  # noqa: E402

logger = logging.getLogger(__name__)
plugin = AgentOSPlugin("multimodal_postprocessor_pipeline")

_instance: MultimodalPostprocessor | None = None


@plugin.on_load
async def _on_load(params: dict) -> None:
    """Initialize multimodal_postprocessor plugin."""
    global _instance
    config = plugin.get_config()
    _instance = MultimodalPostprocessor(config=config)


@plugin.on_unload
async def _on_unload(params: dict) -> None:
    """Cleanup multimodal_postprocessor plugin."""
    global _instance
    _instance = None


@plugin.tool(
    name="multimodal_postprocessor.execute",
    schema={
        "type": "object",
        "properties": {
    ...
    description="Execute Multimodal Postprocessor pipeline plugin",
)
async def execute(state: dict, config: dict | None = None) -> dict:
    """Execute the multimodal_postprocessor pipeline plugin.
