#!/usr/bin/env python3
"""stuck_detector output pipeline plugin MCP 服务端——纯接口适配层。

老代码从 src/plugins/shared/output/stuck_detector/plugin.py 原封不动复制到本目录，
...
from plugin import StuckDetector  # noqa: E402

logger = logging.getLogger(__name__)
plugin = AgentOSPlugin("stuck_detector_pipeline")

_instance: StuckDetector | None = None


@plugin.on_load
async def _on_load(params: dict) -> None:
    """Initialize stuck_detector plugin."""
    global _instance
    config = plugin.get_config()
    _instance = StuckDetector(config=config)


@plugin.on_unload
async def _on_unload(params: dict) -> None:
    """Cleanup stuck_detector plugin."""
    global _instance
    _instance = None


@plugin.tool(
    name="stuck_detector.execute",
    schema={
        "type": "object",
        "properties": {
    ...
    description="Execute Stuck Detector pipeline plugin",
)
async def execute(state: dict, config: dict | None = None) -> dict:
    """Execute the stuck_detector pipeline plugin.
