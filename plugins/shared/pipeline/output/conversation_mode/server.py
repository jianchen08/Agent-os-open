#!/usr/bin/env python3
"""conversation_mode output pipeline plugin MCP 服务端——纯接口适配层。

老代码从 src/plugins/shared/output/conversation_mode/plugin.py 原封不动复制到本目录，
...
from plugin import ConversationModeDetector  # noqa: E402

logger = logging.getLogger(__name__)
plugin = AgentOSPlugin("conversation_mode_pipeline")

_instance: ConversationModeDetector | None = None


@plugin.on_load
async def _on_load(params: dict) -> None:
    """Initialize conversation_mode plugin."""
    global _instance
    config = plugin.get_config()
    _instance = ConversationModeDetector(config=config)


@plugin.on_unload
async def _on_unload(params: dict) -> None:
    """Cleanup conversation_mode plugin."""
    global _instance
    _instance = None


@plugin.tool(
    name="conversation_mode.execute",
    schema={
        "type": "object",
        "properties": {
    ...
    description="Execute Conversation Mode pipeline plugin",
)
async def execute(state: dict, config: dict | None = None) -> dict:
    """Execute the conversation_mode pipeline plugin.
