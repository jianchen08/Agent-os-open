#!/usr/bin/env python3
"""delegate_depth_guard output pipeline plugin MCP 服务端——纯接口适配层。

老代码从 src/plugins/shared/output/delegate_depth_guard/plugin.py 原封不动复制到本目录，
...
from plugin import DelegateDepthGuardPlugin  # noqa: E402

logger = logging.getLogger(__name__)
plugin = AgentOSPlugin("delegate_depth_guard_pipeline")

_instance: DelegateDepthGuardPlugin | None = None


@plugin.on_load
async def _on_load(params: dict) -> None:
    """Initialize delegate_depth_guard plugin."""
    global _instance
    config = plugin.get_config()
    _instance = DelegateDepthGuardPlugin(config=config)


@plugin.on_unload
async def _on_unload(params: dict) -> None:
    """Cleanup delegate_depth_guard plugin."""
    global _instance
    _instance = None


@plugin.tool(
    name="delegate_depth_guard.execute",
    schema={
        "type": "object",
        "properties": {
    ...
    description="Execute Delegate Depth Guard pipeline plugin",
)
async def execute(state: dict, config: dict | None = None) -> dict:
    """Execute the delegate_depth_guard pipeline plugin.
