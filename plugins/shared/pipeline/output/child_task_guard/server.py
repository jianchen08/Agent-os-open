#!/usr/bin/env python3
"""child_task_guard output pipeline plugin MCP 服务端——纯接口适配层。

老代码从 src/plugins/shared/output/child_task_guard/plugin.py 原封不动复制到本目录，
...
# `from tasks.service_access import get_task_service` 的解析前提），
# 不靠同宿插件副作用。
bootstrap_plugin(__file__, extra=('system',))

from plugin import ChildTaskGuard, set_state_reader  # noqa: E402

from agentos_plugin_sdk import AgentOSPlugin  # noqa: E402

logger = logging.getLogger(__name__)
plugin = AgentOSPlugin("child_task_guard_pipeline")


@lru_cache(maxsize=1)
def get_instance() -> ChildTaskGuard:
    """懒构建并缓存插件单例（线程安全；替代模块级可变 `_instance` 全局）。"""
    config = plugin.get_config()
    return ChildTaskGuard(config=config)
...
async def _on_unload(params: dict) -> None:
    """Cleanup child_task_guard plugin."""
    get_instance.cache_clear()


@plugin.tool(
    name="child_task_guard.execute",
    schema={
        "type": "object",
        "properties": {
    ...
    description="Execute Child Task Guard pipeline plugin",
)
async def execute(state: dict, config: dict | None = None) -> dict:
    """Execute the child_task_guard pipeline plugin.
