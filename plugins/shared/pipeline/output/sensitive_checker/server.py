#!/usr/bin/env python3
"""sensitive_checker output pipeline plugin MCP 服务端——纯接口适配层。

老代码从 src/plugins/shared/output/sensitive_checker/plugin.py 原封不动复制到本目录，
...
from plugin import SensitiveChecker  # noqa: E402

logger = logging.getLogger(__name__)
plugin = AgentOSPlugin("sensitive_checker_pipeline")

_instance: SensitiveChecker | None = None


@plugin.on_load
async def _on_load(params: dict) -> None:
    """Initialize sensitive_checker plugin."""
    global _instance
    config = plugin.get_config()
    _instance = SensitiveChecker(config=config)


@plugin.on_unload
async def _on_unload(params: dict) -> None:
    """Cleanup sensitive_checker plugin."""
    global _instance
    _instance = None


@plugin.tool(
    name="sensitive_checker.execute",
    schema={
        "type": "object",
        "properties": {
    ...
    description="Execute Sensitive Checker pipeline plugin",
)
async def execute(state: dict, config: dict | None = None) -> dict:
    """Execute the sensitive_checker pipeline plugin.
