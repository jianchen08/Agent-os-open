#!/usr/bin/env python3
"""llm_error_recovery output pipeline plugin MCP 服务端——纯接口适配层。

老代码从 src/plugins/shared/output/llm_error_recovery/plugin.py 原封不动复制到本目录，
...
from plugin import LLMErrorRecoveryPlugin  # noqa: E402

logger = logging.getLogger(__name__)
plugin = AgentOSPlugin("llm_error_recovery_pipeline")

_instance: LLMErrorRecoveryPlugin | None = None


@plugin.on_load
async def _on_load(params: dict) -> None:
    """Initialize llm_error_recovery plugin."""
    global _instance
    config = plugin.get_config()
    _instance = LLMErrorRecoveryPlugin(config=config)


@plugin.on_unload
async def _on_unload(params: dict) -> None:
    """Cleanup llm_error_recovery plugin."""
    global _instance
    _instance = None


@plugin.tool(
    name="llm_error_recovery.execute",
    schema={
        "type": "object",
        "properties": {
    ...
    description="Execute LLM Error Recovery pipeline plugin",
)
async def execute(state: dict, config: dict | None = None) -> dict:
    """Execute the llm_error_recovery pipeline plugin.
