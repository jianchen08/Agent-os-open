#!/usr/bin/env python3
"""output_repetition_guard output pipeline plugin MCP 服务端——纯接口适配层。

老代码从 src/plugins/shared/output/output_repetition_guard/plugin.py 原封不动复制到本目录，
...
from plugin import OutputRepetitionGuard  # noqa: E402

logger = logging.getLogger(__name__)
plugin = AgentOSPlugin("output_repetition_guard_pipeline")

_instance: OutputRepetitionGuard | None = None


@plugin.on_load
async def _on_load(params: dict) -> None:
    """Initialize output_repetition_guard plugin."""
    global _instance
    config = plugin.get_config()
    _instance = OutputRepetitionGuard(config=config)


@plugin.on_unload
async def _on_unload(params: dict) -> None:
    """Cleanup output_repetition_guard plugin."""
    global _instance
    _instance = None


@plugin.tool(
    name="output_repetition_guard.execute",
    schema={
        "type": "object",
        "properties": {
    ...
    description="Execute Output Repetition Guard pipeline plugin",
)
async def execute(state: dict, config: dict | None = None) -> dict:
    """Execute the output_repetition_guard pipeline plugin.
