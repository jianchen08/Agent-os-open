#!/usr/bin/env python3
"""error_check output pipeline plugin MCP 服务端——纯接口适配层。

老代码从 src/plugins/shared/output/error_check/plugin.py 原封不动复制到本目录，
...
from plugin import ErrorCheckPlugin  # noqa: E402

logger = logging.getLogger(__name__)
plugin = AgentOSPlugin("error_check_pipeline")

_instance: ErrorCheckPlugin | None = None


@plugin.on_load
async def _on_load(params: dict) -> None:
    """Initialize error_check plugin."""
    global _instance
    config = plugin.get_config()
    _instance = ErrorCheckPlugin(config=config)


@plugin.on_unload
async def _on_unload(params: dict) -> None:
    """Cleanup error_check plugin."""
    global _instance
    _instance = None


@plugin.tool(
    name="error_check.execute",
    schema={
        "type": "object",
        "properties": {
    ...
    description="Execute Error Check pipeline plugin",
)
async def execute(state: dict, config: dict | None = None) -> dict:
    """Execute the error_check pipeline plugin.
