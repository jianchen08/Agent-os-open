#!/usr/bin/env python3
"""track output pipeline plugin MCP 服务端——纯接口适配层。

老代码从 src/plugins/shared/output/track/plugin.py 原封不动复制到本目录，
...
_instance: TrackPlugin | None = None


@plugin.on_load
async def _on_load(params: dict) -> None:
    """Initialize track plugin."""
    global _instance
    config = plugin.get_config()
    _instance = TrackPlugin(config=config)


@plugin.on_unload
async def _on_unload(params: dict) -> None:
    """Cleanup track plugin."""
    global _instance
    _instance = None
...
@plugin.tool(
    ...
    description="Execute Track pipeline plugin",
)
async def execute(state: dict, config: dict | None = None) -> dict:
    """Execute the track pipeline plugin.
