#!/usr/bin/env python3
"""experience_consolidator output pipeline plugin MCP 服务端——纯接口适配层。

老代码从 src/plugins/shared/output/experience_consolidator/plugin.py 原封不动复制到本目录，
...
_instance: ExperienceConsolidatorPlugin | None = None


@plugin.on_load
async def _on_load(params: dict) -> None:
    """Initialize experience_consolidator plugin + 注入记忆后端。"""
    global _instance
    config = plugin.get_config()
    _instance = ExperienceConsolidatorPlugin(config=config)
    backend = build_memory_backend(plugin)
    if backend:
        set_memory_backend(backend)
    else:
        logger.warning("[experience_consolidator_pipeline] 记忆后端未注入，功能降级")


@plugin.on_unload
async def _on_unload(params: dict) -> None:
    """Cleanup experience_consolidator plugin."""
    global _instance
    _instance = None


@plugin.tool(
    name="experience_consolidator.execute",
    schema={
        "type": "object",
        "properties": {
    ...
    description="Execute Experience Consolidator pipeline plugin",
)
async def execute(state: dict, config: dict | None = None) -> dict:
    """Execute the experience_consolidator pipeline plugin.
