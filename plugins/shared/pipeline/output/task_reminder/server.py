#!/usr/bin/env python3
"""task_reminder output pipeline plugin MCP 服务端——纯接口适配层。

老代码从 src/plugins/shared/output/task_reminder/plugin.py 原封不动复制到本目录，
...
plugin = AgentOSPlugin("task_reminder_pipeline")

_instance: TaskReminder | None = None


@plugin.on_load
async def _on_load(params: dict) -> None:
    """Initialize task_reminder plugin."""
    global _instance
    config = plugin.get_config()
    _instance = TaskReminder(config=config)


@plugin.on_unload
async def _on_unload(params: dict) -> None:
    """Cleanup task_reminder plugin."""
    global _instance
    _instance = None


@plugin.tool(
    name="task_reminder.execute",
    schema={
        "type": "object",
        "properties": {
    ...
    description="Execute Task Reminder pipeline plugin",
)
async def execute(state: dict, config: dict | None = None) -> dict:
    """Execute the task_reminder pipeline plugin.
