#!/usr/bin/env python3
"""pending_tools output pipeline plugin MCP 服务端——纯接口适配层。

老代码从 src/plugins/shared/output/pending_tools/plugin.py 原封不动复制到本目录，
...
from plugin import PendingToolsOutput  # noqa: E402

logger = logging.getLogger(__name__)
plugin = AgentOSPlugin("pending_tools_pipeline")

_instance: PendingToolsOutput | None = None


@plugin.on_load
async def _on_load(params: dict) -> None:
    """Initialize pending_tools plugin."""
    global _instance
    config = plugin.get_config()
    _instance = PendingToolsOutput(config=config)


@plugin.on_unload
async def _on_unload(params: dict) -> None:
    """Cleanup pending_tools plugin."""
    global _instance
    _instance = None


@plugin.tool(
    name="pending_tools.execute",
    schema={
        "type": "object",
        "properties": {
    ...
    description="Execute Pending Tools pipeline plugin",
)
async def execute(state: dict, config: dict | None = None) -> dict:
    """Execute the pending_tools pipeline plugin.
