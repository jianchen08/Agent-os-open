#!/usr/bin/env python3
"""approval_view_route output pipeline plugin MCP 服务端——纯接口适配层。

老代码从 src/plugins/shared/output/approval_view_route/plugin.py 原封不动复制到本目录，
...
from plugin import ApprovalViewRoutePlugin  # noqa: E402

logger = logging.getLogger(__name__)
plugin = AgentOSPlugin("approval_view_route_pipeline")

_instance: ApprovalViewRoutePlugin | None = None


@plugin.on_load
async def _on_load(params: dict) -> None:
    """Initialize approval_view_route plugin."""
    global _instance
    config = plugin.get_config()
    _instance = ApprovalViewRoutePlugin(config=config)


@plugin.on_unload
async def _on_unload(params: dict) -> None:
    """Cleanup approval_view_route plugin."""
    global _instance
    _instance = None


@plugin.tool(
    name="approval_view_route.execute",
    schema={
        "type": "object",
        "properties": {
    ...
    description="Execute Approval View Route pipeline plugin",
)
async def execute(state: dict, config: dict | None = None) -> dict:
    """Execute the approval_view_route pipeline plugin.
