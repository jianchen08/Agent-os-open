#!/usr/bin/env python3
"""tool_context input pipeline plugin MCP 服务端——纯接口适配层。

老代码从 src/plugins/shared/input/tool_context/plugin.py 原封不动复制到本目录，
...
plugin = AgentOSPlugin("tool_context_pipeline")

_instance: ToolContextPlugin | None = None


@plugin.on_load
async def _on_load(params: dict) -> None:
    """Initialize tool_context plugin."""
    global _instance
    config = plugin.get_config()
    _instance = ToolContextPlugin(config=config)


@plugin.on_unload
async def _on_unload(params: dict) -> None:
    """Cleanup tool_context plugin."""
    global _instance
    _instance = None


@plugin.tool(
    name="tool_context.execute",
    schema={
        "type": "object",
        "properties": {
    ...
    description="Execute Tool Context pipeline plugin",
)
async def execute(state: dict, config: dict | None = None) -> dict:
    """Execute the tool_context pipeline plugin.
