...
_instance: ParamInjectPlugin | None = None
...
async def _on_load(params: dict) -> None:
    ...
    _instance = ParamInjectPlugin(config=config)


@plugin.on_unload
async def _on_unload(params: dict) -> None:
    """Cleanup param_inject plugin."""
    global _instance
    _instance = None


@plugin.tool(
    name="param_inject.execute",
    schema={
        "type": "object",
        "properties": {
    ...
    description="Execute Param Inject pipeline plugin",
)
async def execute(state: dict, config: dict | None = None) -> dict:
    """Execute the param_inject pipeline plugin.
