#!/usr/bin/env python3
"""injected_param_validator input pipeline plugin MCP 服务端——纯接口适配层。

老代码从 src/plugins/shared/input/injected_param_validator/plugin.py 原封不动复制到本目录，
...
from plugin import InjectedParamValidator  # noqa: E402

logger = logging.getLogger(__name__)
plugin = AgentOSPlugin("injected_param_validator_pipeline")

_instance: InjectedParamValidator | None = None


@plugin.on_load
async def _on_load(params: dict) -> None:
    """Initialize injected_param_validator plugin."""
    global _instance
    config = plugin.get_config()
    _instance = InjectedParamValidator(config=config)


@plugin.on_unload
async def _on_unload(params: dict) -> None:
    """Cleanup injected_param_validator plugin."""
    global _instance
    _instance = None


@plugin.tool(
    name="injected_param_validator.execute",
    schema={
        "type": "object",
        "properties": {
    ...
    description="Execute Injected Param Validator pipeline plugin",
)
async def execute(state: dict, config: dict | None = None) -> dict:
    """Execute the injected_param_validator pipeline plugin.
