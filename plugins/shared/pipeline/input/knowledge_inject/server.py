#!/usr/bin/env python3
"""knowledge_inject input pipeline plugin MCP 服务端——纯接口适配层。

老代码从 src/plugins/shared/input/knowledge_inject/plugin.py 原封不动复制到本目录，
...
from plugin import KnowledgeInjectPlugin, set_memory_backend  # noqa: E402
...
plugin = AgentOSPlugin("knowledge_inject_pipeline")

_instance: KnowledgeInjectPlugin | None = None


@plugin.on_load
async def _on_load(params: dict) -> None:
    """Initialize knowledge_inject plugin + 注入记忆后端。"""
    global _instance
    config = plugin.get_config()
    _instance = KnowledgeInjectPlugin(config=config)
    backend = build_memory_backend(plugin)
    if backend:
        set_memory_backend(backend)
    else:
        logger.warning("[knowledge_inject_pipeline] 记忆后端未注入，功能降级")


@plugin.on_unload
async def _on_unload(params: dict) -> None:
    """Cleanup knowledge_inject plugin."""
    global _instance
    _instance = None


@plugin.tool(
    name="knowledge_inject.execute",
    schema={
        "type": "object",
        "properties": {
    ...
    description="Execute Knowledge Inject pipeline plugin",
)
async def execute(state: dict, config: dict | None = None) -> dict:
    """Execute the knowledge_inject pipeline plugin.
