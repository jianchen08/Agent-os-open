#!/usr/bin/env python3
"""memory_read input pipeline plugin MCP 服务端——纯接口适配层。

老代码从 src/plugins/shared/input/memory_read/plugin.py 原封不动复制到本目录，
...
from plugin import MemoryReadPlugin, set_capability_caller, set_memory_backend  # noqa: E402
...
@lru_cache(maxsize=1)
def get_instance() -> MemoryReadPlugin:
    """懒构建并缓存插件单例（线程安全；替代模块级可变 `_instance` 全局）。"""
    config = plugin.get_config()
    return MemoryReadPlugin(config=config)
...
async def _on_unload(params: dict) -> None:
    """Cleanup memory_read plugin."""
    get_instance.cache_clear()


@plugin.tool(
    name="memory_read.execute",
    schema={
        "type": "object",
        "properties": {
    ...
    description="Execute Memory Read pipeline plugin",
)
async def execute(state: dict, config: dict | None = None) -> dict:
    """Execute the memory_read pipeline plugin.
