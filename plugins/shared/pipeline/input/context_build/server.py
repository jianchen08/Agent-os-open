#!/usr/bin/env python3
"""context_build input pipeline plugin MCP 服务端——纯接口适配层。

老代码从 src/plugins/shared/input/context_build/plugin.py 原封不动复制到本目录，
...
_instance: ContextBuildPlugin | None = None


@plugin.on_load
async def _on_load(params: dict) -> None:
    """Initialize context_build plugin."""
    global _instance
    config = plugin.get_config()
    _instance = ContextBuildPlugin(config=config)


@plugin.on_unload
async def _on_unload(params: dict) -> None:
    """Cleanup context_build plugin."""
    global _instance
    _instance = None


@plugin.tool(
    name="context_build.execute",
    schema={
        "type": "object",
        "properties": {
...
async def execute(state: dict, config: dict | None = None) -> dict:
    """Execute the context_build pipeline plugin.
