#!/usr/bin/env python3
"""cost_control input pipeline plugin MCP 服务端——纯接口适配层。

老代码从 src/plugins/shared/input/cost_control/plugin.py 原封不动复制到本目录，
...
from plugin import CostControlPlugin  # noqa: E402

logger = logging.getLogger(__name__)
plugin = AgentOSPlugin("cost_control_pipeline")

_instance: CostControlPlugin | None = None


@plugin.on_load
async def _on_load(params: dict) -> None:
    """Initialize cost_control plugin."""
    global _instance
    config = plugin.get_config()
    _instance = CostControlPlugin(config=config)


@plugin.on_unload
async def _on_unload(params: dict) -> None:
    """Cleanup cost_control plugin."""
    global _instance
    _instance = None


@plugin.tool(
    name="cost_control.execute",
    schema={
        "type": "object",
        "properties": {
    ...
    description="Execute Cost Control pipeline plugin",
)
async def execute(state: dict, config: dict | None = None) -> dict:
    """Execute the cost_control pipeline plugin.
