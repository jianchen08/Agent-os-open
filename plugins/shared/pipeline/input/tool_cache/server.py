#!/usr/bin/env python3
"""tool_cache input pipeline plugin MCP 服务端——纯接口适配层。

老代码从 src/plugins/shared/input/tool_cache/plugin.py 原封不动复制到本目录，
...
from plugin import ToolCache  # noqa: E402

logger = logging.getLogger(__name__)
plugin = AgentOSPlugin("tool_cache_pipeline")


@lru_cache(maxsize=1)
def get_instance() -> ToolCache:
    """懒构建并缓存插件单例（线程安全；替代模块级可变 `_instance` 全局）。"""
    config = plugin.get_config()
    return ToolCache(config=config)


@plugin.on_load
async def _on_load(params: dict) -> None:
    """Initialize tool_cache plugin."""
    get_instance()  # 启动时预热，保持原 on_load 构造时机


@plugin.on_unload
async def _on_unload(params: dict) -> None:
    """Cleanup tool_cache plugin."""
    get_instance.cache_clear()


@plugin.tool(
    name="tool_cache.execute",
    schema={
        "type": "object",
        "properties": {
    ...
    description="Execute Tool Cache pipeline plugin",
)
async def execute(state: dict, config: dict | None = None) -> dict:
    """Execute the tool_cache pipeline plugin.
