#!/usr/bin/env python3
"""task_event_receiver input pipeline plugin MCP 服务端——纯接口适配层。

老代码从 src/plugins/shared/input/task_event_receiver/plugin.py 原封不动复制到本目录，
...
from plugin import TaskEventReceiverPlugin  # noqa: E402

logger = logging.getLogger(__name__)
plugin = AgentOSPlugin("task_event_receiver_pipeline")

_instance: TaskEventReceiverPlugin | None = None


@plugin.on_load
async def _on_load(params: dict) -> None:
    """Initialize task_event_receiver plugin."""
    global _instance
    config = plugin.get_config()
    _instance = TaskEventReceiverPlugin(config=config)


@plugin.on_unload
async def _on_unload(params: dict) -> None:
    """Cleanup task_event_receiver plugin."""
    global _instance
    _instance = None


@plugin.tool(
    name="task_event_receiver.execute",
    schema={
        "type": "object",
        "properties": {
    ...
    description="Execute Task Event Receiver pipeline plugin",
)
async def execute(state: dict, config: dict | None = None) -> dict:
    """Execute the task_event_receiver pipeline plugin.
