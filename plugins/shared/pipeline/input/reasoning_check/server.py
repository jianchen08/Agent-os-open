#!/usr/bin/env python3
"""reasoning_check input pipeline plugin MCP 服务端——纯接口适配层。

老代码从 src/plugins/shared/input/reasoning_check/plugin.py 原封不动复制到本目录，
...
from plugin import ReasoningCheckPlugin  # noqa: E402

logger = logging.getLogger(__name__)
plugin = AgentOSPlugin("reasoning_check_pipeline")

_instance: ReasoningCheckPlugin | None = None


@plugin.on_load
async def _on_load(params: dict) -> None:
    """Initialize reasoning_check plugin."""
    global _instance
    config = plugin.get_config()
    _instance = ReasoningCheckPlugin(config=config)


@plugin.on_unload
async def _on_unload(params: dict) -> None:
    """Cleanup reasoning_check plugin."""
    global _instance
    _instance = None


@plugin.tool(
    name="reasoning_check.execute",
    schema={
        "type": "object",
        "properties": {
    ...
    description="Execute Reasoning Check pipeline plugin",
)
async def execute(state: dict, config: dict | None = None) -> dict:
    """Execute the reasoning_check pipeline plugin.
