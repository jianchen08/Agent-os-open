...
"""level_guard server.py（MCP 接口适配层）单元测试。

覆盖：懒构建单例缓存与配置注入、on_load 预热、on_unload 清缓存、
execute 工具入口（dict 直通、PluginResult → state_updates 序列化、
route_signal / skip_remaining 透传、create_initial_state 状态合并）。

server.py 以唯一模块名动态加载（importlib 显式路径），加载前逐出裸名
``plugin`` 防兄弟插件目录串扰；``plugin.get_config`` 为 SDK 外部依赖，
以 monkeypatch 注入配置。

[来源: plugins/shared/pipeline/input/level_guard/server.py]
"""
...
def test_execute_dict_result_passthrough(monkeypatch: pytest.MonkeyPatch) -> None:
    server = _load_server()
    monkeypatch.setattr(server.plugin, "get_config", lambda: {})

    async def _fake_execute(ctx: Any) -> dict:
        return {"security.level_decision": {"allowed": False, "reason": "blocked"}}

    monkeypatch.setattr(server.get_instance(), "execute", _fake_execute)
    data = _run(server.execute({"core_type": "tool_execute"}))
    assert data == {"security.level_decision": {"allowed": False, "reason": "blocked"}}


def test_execute_plugin_result_serialized(monkeypatch: pytest.MonkeyPatch) -> None:
    server = _load_server()
    monkeypatch.setattr(server.plugin, "get_config", lambda: {})

    class _FakeResult:
        state_updates = {"security.level_decision": {"allowed": True, "reason": "ok"}}
        route_signal = None
...
def test_execute_merges_state_via_create_initial_state(monkeypatch: pytest.MonkeyPatch) -> None:
    server = _load_server()
    monkeypatch.setattr(server.plugin, "get_config", lambda: {})
    captured: dict = {}
    ...
    monkeypatch.setattr(server.get_instance(), "execute", _fake_execute)
    _run(server.execute({"core_type": "tool_execute", "raw_tool_calls": [{"name": "task_submit"}]}))
    assert captured["state"]["core_type"] == "tool_execute"
    assert captured["state"]["raw_tool_calls"] == [{"name": "task_submit"}]
