"""Host Context——通用宿主选中引用 Input 插件。

灵汐 AgentOS 与外部宿主（Godot 4 / VSCode / Blender …）的选中对象引用桥
（事件驱动，无轮询）。连接者身份不烧死在代码里，由 manifest 配置指定：

    - source           : 引用块来源标识（<reference source="...">，缺省 godot）
    - display_name     : 前端镜像行显示名（缺省回退 source）
    - preview_endpoint : 宿主预览代理地址（空 = 关闭预览）

宿主插件（如 hosts/godot-addons/agentos）在选中变化信号时 POST 推送到本插件
/ext/pipeline_host_context/selection 端点，本插件负责三件事：

1. 维护最新选中快照（心跳 15s 内视为在线）；
2. 选中变化时经 FrontendEmitter 向订阅线程 emit ``host_selection_changed``
...
State 命名空间：
    - host.injected_for : 已合并引用的 message_id（幂等去重）。

引用清理（dismiss）：前端可请求清除当前引用（用户不想让这条选中随消息注入）。
清理后被清理签名的**心跳**被抑制——宿主里对象仍选中时 5s 心跳不会把引用带
...
class HostContextPlugin(IInputPlugin):
    """外部宿主选中上下文：快照缓存 + 前端转发 + 消息级引用注入。

    连接者身份（source/display_name/preview_endpoint）来自 manifest 配置，
    经插件热重载（respawn 重建实例）生效。
    """

    HEARTBEAT_STALE_MS = 15_000  # 超过 15s 无心跳视为宿主离线
    ...
    def __init__(self, config: dict[str, Any] | None = None):
        cfg = dict(config or {})
        # 内核按 config_files[].id 命名空间注入（{"settings": {...}}）；直构
        # （测试/兜底）允许平铺。两形态兼容读，缺省值与 manifest fields 一致。
        namespace = cfg.get("settings")
        if isinstance(namespace, dict):
            cfg = namespace
        self._source = str(cfg.get("source") or "godot")
        self._display_name = str(cfg.get("display_name") or "") or self._source
        self._preview_endpoint = str(cfg.get("preview_endpoint") or "")
    ...
    def snapshot(self) -> dict[str, Any]:
        ...
        source/display_name 随快照下发——前端 provider 据此动态渲染标签与
        引用块来源，不烧死任何宿主名。
        """
        snap = dict(self._snapshot)
        snap["source"] = self._source
        snap["display_name"] = self._display_name
    ...
    async def dismiss(self) -> dict[str, Any]:
        ...
        connected/scene 保留（宿主仍在线）；签名变化（改选/取消选中）恢复。
        空引用时 no-op（幂等）。
        """
    ...
    async def _broadcast(self) -> None:
        ...
        for thread_id in list(self._subscribed_threads):
            try:
                ...
                logger.warning("[host_context] emit %s 失败（继续）: %s", thread_id, e)
    ...
    async def execute(self, ctx: PluginContext) -> PluginResult:
        ...
        if not message_id or state.get("host.injected_for") == message_id:
            return PluginResult()
        ...
        # 回环判定覆盖任意 source（历史消息可能携带旧配置写入的其他来源），
        # 与前端 buildReferenceBlock 共用 '<reference source="' 前缀形态。
        if seq is None or '<reference source="' in content:
            return PluginResult()
    ...
    def _build_reference_content(self, snap: dict[str, Any]) -> str:
        scene = snap.get("scene") or {}
        lines = [f'<reference source="{self._source}" scene="{scene.get("path", "")}">']
