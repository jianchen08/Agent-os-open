#!/usr/bin/env python3
"""tool_call_guard input pipeline plugin MCP 服务端——纯接口适配层。

老代码从 src/plugins/shared/input/tool_call_guard/plugin.py 原封不动复制到本目录，
...
from plugin import ToolCallGuard  # noqa: E402

logger = logging.getLogger(__name__)
plugin = AgentOSPlugin("tool_call_guard_pipeline")

_instance: ToolCallGuard | None = None


@plugin.on_load
async def _on_load(params: dict) -> None:
    """Initialize tool_call_guard plugin."""
    global _instance
    config = plugin.get_config()
    _instance = ToolCallGuard(config=config)


@plugin.on_unload
async def _on_unload(params: dict) -> None:
    """Cleanup tool_call_guard plugin."""
    global _instance
    _instance = None


@plugin.tool(
    name="tool_call_guard.execute",
    schema={
        "type": "object",
        "properties": {
    ...
    description="Execute Tool Call Guard pipeline plugin",
)
async def execute(state: dict, config: dict | None = None) -> dict:
    """Execute the tool_call_guard pipeline plugin.
