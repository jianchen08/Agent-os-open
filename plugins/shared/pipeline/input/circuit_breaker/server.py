#!/usr/bin/env python3
"""circuit_breaker input pipeline plugin MCP 服务端——纯接口适配层。

老代码从 src/plugins/shared/input/circuit_breaker/plugin.py 原封不动复制到本目录，
...
from plugin import CircuitBreaker  # noqa: E402

logger = logging.getLogger(__name__)
plugin = AgentOSPlugin("circuit_breaker_pipeline")
...
async def _on_load(params: dict) -> None:
    """Initialize circuit_breaker plugin."""
    global _instance
    config = plugin.get_config()
    _instance = CircuitBreaker(config=config)


@plugin.on_unload
async def _on_unload(params: dict) -> None:
    """Cleanup circuit_breaker plugin."""
    global _instance
    _instance = None


@plugin.tool(
    name="circuit_breaker.execute",
    schema={
        "type": "object",
        "properties": {
    ...
    description="Execute Circuit Breaker pipeline plugin",
)
async def execute(state: dict, config: dict | None = None) -> dict:
    """Execute the circuit_breaker pipeline plugin.
