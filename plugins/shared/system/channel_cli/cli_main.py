...
# upgrade: 当 pipeline 引擎迁移为独立插件后，通过 MCP 调用替换直接 import。
try:
    ...
    PipelineEngine = None  # type: ignore[assignment,misc]
    PluginRegistry = None  # type: ignore[assignment,misc]
    InputRouteTable = None  # type: ignore[assignment,misc]
    OutputRouteTable = None  # type: ignore[assignment,misc]
...
class CLIApplication(CLIRunnerMixin, CLISingleMixin, CLIInteractiveMixin):
    ...
    def _build_services(self, agent_registry: Any = None) -> dict[str, Any]:
        ...
        try:
            from cli_interaction import CLIInteractionNotifier  # noqa: PLC0415
