...
class ImageReviewer:
    ...
    @staticmethod
    def _extract_exif(img: Image.Image) -> dict[str, str | float | int]:  # type: ignore[misc]
        """提取图片 EXIF 元数据。
...
class VideoReviewer:
    ...
    def review(
        ...
        # _extract_metadata 返回 dict[str, str | float | int]，此处按各键
        # 实际类型做运行时空操作 cast（mypy 收紧，值不变——0.1 源同构）。
        fmt = cast(str, metadata_result["format"])
        duration = cast(float, metadata_result["duration_seconds"])
        width = cast(int, metadata_result["width"])
        height = cast(int, metadata_result["height"])
        fps = cast(float, metadata_result["fps"])
        codec = cast(str, metadata_result["codec"])
