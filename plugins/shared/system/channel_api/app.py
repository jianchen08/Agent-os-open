...
from deps import APIError, api_error_handler, generic_error_handler
...
def _add_middleware(app: FastAPI) -> None:
    ...
    async def rate_limit_and_log(
        ...
        with LogContext.scoped(request_id=request_id, trace_id=trace_id):
            ...
            from rate_limiter import tiered_rate_limiter  # noqa: PLC0415
...
def _register_routes(app: FastAPI) -> None:  # noqa: PLR0915
    ...
    # ---- 模块数据路由（手动注册的自定义端点） ----
    from routes_ui import get_module_data_router  # noqa: PLC0415

    app.include_router(get_module_data_router())

    # ---- 自动注册 Data CRUD 路由（基于 YAML data 声明） ----
    from routes_ui import register_data_crud_routes  # noqa: PLC0415

    for crud_router in register_data_crud_routes():
        app.include_router(crud_router)

    # ---- 补全缺失路由（前端期望但之前未注册） ----
    from routes_artifacts import (  # noqa: PLC0415
        annotations_router_v1,
        artifacts_router,
    )
    from routes_asr import asr_router  # noqa: PLC0415
    ...
    from routes_reviews import reviews_router  # noqa: PLC0415
    from routes_workspaces import workspaces_router  # noqa: PLC0415
    ...
    # ---- ComfyUI 路由（新增） ----
    from routes_comfyui import router as comfyui_router  # noqa: PLC0415

    app.include_router(comfyui_router)

    # ---- 场景管理路由 ----
    from routes_scene import router as scene_router  # noqa: PLC0415
