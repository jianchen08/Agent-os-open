...
使用 lingxi-plugin-sdk 的 AgentOSPlugin 注册全部工具，
...
from lingxi_builtin_tools.bash_tool import BASH_EXECUTE_SCHEMA, bash_execute
...
from lingxi_builtin_tools.search_tool import ENHANCED_SEARCH_SCHEMA, enhanced_search
...
def create_plugin() -> AgentOSPlugin:
    ...
    plugin = AgentOSPlugin("lingxi-builtin-tools")

    plugin.register_tool("file_read", FILE_READ_SCHEMA, file_read, "读取文件内容")
    plugin.register_tool("file_write", FILE_WRITE_SCHEMA, file_write, "写入/编辑文件")
