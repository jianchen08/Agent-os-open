...
def service(tmp_path: Path) -> Any:
    ...
    ``from service import`` 前把 tasks 插件目录强制置顶：车道共跑时
    security_check 等插件的同名 service.py 可能占住 path 前位，导致这里
    拿到错误实现（ImportError: cannot import name 'TaskService'）。只调整
    解析顺序、不替换模块对象，避免 tool.py 侧类身份错位。
    """
    _s = str(_TASKS_DIR)
