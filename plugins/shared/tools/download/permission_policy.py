"""
权限策略管理器（download 插件自带镜像）

镜像自 plugins/shared/system/isolation/permission_policy.py——download sidecar
进程无法 import 其他插件的包（isolation.* 仅 system 插件目录可解析），路径
权限校验必须随插件自带。两副本需保持同步：以 system/isolation 为源，改任何
一边必须同步另一边（复制收敛见扫描报告 20260827 批D）。
