...
from tasks._task_cleanup import _TaskCleanupMixin
from tasks._task_crud import _TaskCrudMixin
from tasks._task_state import _TaskStateMixin
...
class TaskService(_TaskCrudMixin, _TaskStateMixin, _TaskCleanupMixin):
    ...
    async def _emit_state_change(
        ...
        # 绑定日志上下文，使后续日志自动携带 task_id（compat shim 不携带
        # core.logging，缺失时静默跳过——日志上下文是开发体验项，非功能性）
        try:
