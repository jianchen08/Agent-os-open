...
isolation_guard / security_check（pipeline input）与 isolation 服务
（system）共用的 isolation_policy.yaml 加载与决策逻辑集中于此，
消除三份近逐字副本的锚点漂移。
...
暴露接口：
- default_policy_path()：策略文件锚点解析（AGENTOS_CONFIG_ROOT 优先，
  回退祖先目录探测，不硬编码父目录层数）
...
class IsolationPolicyLoader:
    ...
    def _parse_policy(self, data: dict[str, Any]) -> ToolIsolationPolicy:
        """解析单条策略配置
