...
isolation 服务（system）与 download 工具插件共用的路径权限校验集中于此，
消除双副本漂移（CUSTOM 白名单分隔符锚定为此处权威语义）。

暴露接口：
- PermissionChecker：权限检查器类
  - check_read_permission(path, workspace, policy) -> tuple[bool, str]
  - check_write_permission(path, workspace, policy, operation="write") -> tuple[bool, str]
  - is_path_in_workspace(path, workspace) -> tuple[bool, str | None]
  - get_project_root() -> str
  - resolve_path(path) -> str
- check_write_permission(path, workspace, policy, project_root=".", operation="write")
  -> tuple[bool, str]：便捷函数（dict 策略自动转换）
"""
...
class PermissionChecker:
    ...
    def check_read_permission(  # noqa: PLR0911
        ...
        if read_perm.scope == PermissionScope.CUSTOM:
            if read_perm.custom_paths:
                ...
                if self._matches_custom_path(normalized_path, read_perm.custom_paths):
                    return True, ""
    ...
    def check_write_permission(  # noqa: PLR0911
        ...
        if write_perm.scope == PermissionScope.CUSTOM:
            if write_perm.custom_paths:
                normalized_path = self._normalize_path(path)
                if self._matches_custom_path(normalized_path, write_perm.custom_paths):
                    return True, ""
            return False, f"路径 '{path}' 不在允许的自定义路径列表中"
    ...
    def _matches_custom_path(self, normalized_path: str, custom_paths: list[str]) -> bool:
        """路径是否命中 CUSTOM 白名单：等值或真实子路径（前缀需带路径分隔符）。

        纯 startswith 前缀比较会把兄弟目录（custom=…/ws 时 …/ws-evil）误判
        为界内，与 _is_path_inside 同需分隔符锚定。
        """
        for custom_path in custom_paths:
            custom_full = self._normalize_path(custom_path)
            if normalized_path == custom_full or normalized_path.startswith(custom_full + os.sep):
                return True
        return False
...
def check_write_permission(
    ...
    if isinstance(policy, dict):
        from agentos_plugin_sdk.permission_policy import (  # noqa: PLC0415
