#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""浏览器验证 v4：防崩溃版（限制 Chromium 内存 + 耐心等待 React 挂载 + 登录发消息 + 工具卡片轮询）"""
...
with sync_playwright() as p:
    browser = p.chromium.launch(
        ...
        args=[
            ...
            "--js-flags=--max-old-space-size=512",  # 限制 JS 堆防 OOM
    ...
    for i in range(75):
        try:
            ...
            pass
        if i % 5 == 0:
            print(f"[3] 等待 React 挂载... ({i*2}s) errors={len(result['errors'])}", flush=True)
        try:
