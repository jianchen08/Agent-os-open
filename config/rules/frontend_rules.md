...
| 规范 | 说明 |
|------|------|
| 页面对象模式 | 封装页面元素和操作，提高可维护性 |
| 数据测试属性 | 使用 `data-testid` 而非 CSS 选择器 |
| 异步等待 | 使用 `waitFor` 替代固定 `sleep` |
| 测试隔离 | 每个测试独立，不依赖执行顺序 |
...
- 虚拟滚动：列表数据量大时使用
- 避免渲染函数中创建新函数或对象
- 使用 React.memo/Vue.memo 避免不必要重渲染
...
- 用户输入必须经过转义后再渲染
- 使用框架默认转义机制
- 禁止 `dangerouslySetInnerHTML`（除非确有必要）
- 富文本使用专业过滤库
