...
| 意图测试 | 验证"行为背后的业务意图没有被改变" | `assert discount_price < original_price`（验证"打折必须比原价低"的意图） |

### 8.2 实践要求

- 测试命名应反映业务意图，不只是功能描述
- 测试注释应说明"为什么这个行为重要"
- 当 AI 改代码时，行为测试只能告诉你输出变了，意图测试才能拦住它改变了你不希望被改变的行为本质
- 关键业务规则的测试应包含意图描述注释，说明该规则存在的业务原因
