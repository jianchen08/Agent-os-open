...
| 单点输入拟合（同一断言仅一组输入，实现可硬编码刷绿） | 同一行为 ≥2 组有区分度输入（参数化展开）+ 性质断言（见 coding_domain_rules §6.5） |
| 全 Mock 无真实链路（关键路径无集成/E2E 验证） | 关键路径至少一条走真实依赖的集成/E2E 断言 |
...
### 8.3 防拟合（anti test-fitting）

测试的写法必须让实现**无法靠硬编码期望值/路径刷绿**（详见 coding_domain_rules §6.5）：

- 同一行为 ≥2 组有区分度输入；关键行为配性质断言（意图断言即性质断言的一种）
- 可枚举输入用 `parametrize` 展开；规律明显的行为用 property-based 随机输入
- 关键路径至少一条走真实依赖（数据库/文件系统/子进程）的断言，禁止全 mock
- 判据：把实现硬编码成测试期望值也能全绿 → 测试无效，必须补约束
