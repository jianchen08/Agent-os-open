...
| 陷阱 | 表现 | 解法 |
|------|------|------|
| 不复现就修复 | 改了代码但不确定 bug 是否存在 | 先复现，再修改 |
| 过早最小化 | 简化输入后 bug 消失 | 用原始输入确认 bug 存在，再逐步简化 |
| 修复症状不修复触发 | 压制了报错但根因仍在 | 用 5 Whys 追到根因 |
| 隔离测试陷阱 | 测试通过但系统仍失败 | 切换到全链路追踪（规则 2） |
| 回归测试假阳性 | 测试通过了但验证的不是根因 | 测试必须验证根因条件（规则 11） |
| 森林盲区 | 反复修复同一位置 | 架构熔断器（规则 10） |
| 相关性=因果性 | 两件事同时发生就认为有关联 | 用排除法确认因果关系 |
