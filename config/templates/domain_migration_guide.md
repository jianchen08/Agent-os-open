...
============================================================
【模板是什么】
将通用工作流程体系迁移到新领域的标准化操作指南。

【模板的作用】
1. 规范迁移流程 — 统一迁移步骤和产出物要求
2. 降低迁移门槛 — 提供完整的检查清单和模板
3. 保证质量一致 — 确保每个领域都达到相同标准
4. 支持快速开荒 — 新领域从零开始的完整路径

【如何使用本模板】
1. 确认迁移目标领域
2. 按阶段顺序执行（调研→规则编写→Agent配置→验证）
3. 每阶段完成后对照检查清单验证
4. 最终产出完整的领域规则+Agent配置+验证报告
...
> | 结论 | 判定条件 | 后续动作 |
> |------|---------|---------|
> | 通过 | 所有必填维度通过，领域规则完整可用 | 迁移完成 |
> | 有条件通过 | 必填维度通过，部分规则待完善 | 标注待完善项，可试用 |
> | 不通过 | 存在必填维度未通过 | 退回补充调研和规则编写 |
>
> **检查维度**：
>
> | 维度 | 检查内容 | 必填 | 通过标准 |
> |------|---------|------|---------|
> | 完整性 | 领域规则文件、Agent配置、审查流程齐全 | 是 | 清单中所有必填项已产出 |
> | 调研充分性 | 标杆采集≥3个，有对比分析 | 是 | 材料已归档，分析文档已产出 |
> | 一致性 | 规则与设计文档一致，注入顺序正确 | 是 | 逐项核对通过 |
> | 可用性 | 配置格式正确，路径引用有效 | 是 | YAML验证通过，文件存在 |
-->
